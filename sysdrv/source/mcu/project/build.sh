...
export TARGET_PRODUCT_DIR=$SDK_ROOT_DIR/rt-thread/bsp/rockchip/$TARGET_PRODUCT/
################################################################################
# Plubic Configure
################################################################################
C_BLACK="\e[30;1m"
...
function build_all() {
	...
	rm -rf ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT/Image
	scons -C ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT -j${RK_JOBS}

	if [ -f "${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT/mkimage.sh" ]; then
		cd ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT
		./mkimage.sh
		cd -
	fi
...
function build_menuconfig() {
	scons -C ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT --menuconfig
}
...
function build_clean(){
	...
	case $param in
		all)
			scons -C ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT -c
...
function build_firmware(){
	...
	local project_dir="${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT"

	# Due to the SDK build.sh script, to be compatible with rv1106, only the
	# rtthread.bin is copied from the mcu directory as the thunderboot firmware.
	# Therefore, in the 1103b project, the firmware needs to be remapped."

	# | RTT Project     |     MCU output     |
	# |-----------------|--------------------|
	# | rtthread.bin    |  rtthread_orig.bin |
	# | Image/hpmcu.bin |  rtthread.bin      |

	cp -rfa $project_dir/rtthread.bin ${RK_PROJECT_OUTPUT_IMAGE}/rtthread_orig.bin

	if [ -d "$project_dir/Image" ]; then
		cp -rfa $project_dir/Image/*.img  ${RK_PROJECT_OUTPUT_IMAGE}

		# FIXME: only fixed to rv1103b, hpmcu.img is for thunderboot.
		ln -rsf ${RK_PROJECT_OUTPUT_IMAGE}/hpmcu.img ${RK_PROJECT_OUTPUT_IMAGE}/rtthread.bin
	fi

	if [ -d "$1" ];then
		cp -f ${RK_PROJECT_OUTPUT_IMAGE}/rtthread.bin $1
	fi
...
function build_save(){
	...
	test -d ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT/ && \
		(cd ${SDK_ROOT_DIR}/rt-thread/bsp/rockchip/$TARGET_PRODUCT/ && \
...
if [ ! -f "${TARGET_PRODUCT_DIR}/rtconfig.h" ]; then
	echo "Not found ${TARGET_PRODUCT_DIR}/rtconfig.h"
	echo "Please run $cmd lunch"
	exit 1
fi
