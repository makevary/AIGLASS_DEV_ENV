...
int aupipe_create_link_by_json(Aupipe *aupipe, char *buf)
{
    int element_num;
    cJSON *root = NULL;
    cJSON *_root = NULL;
...
int aupipe_create_link(Aupipe *aupipe, char *args)
{
    ...
    ret = aupipe_create_link_by_json(aupipe, buf);
...
int aupipe_parse_array(char *string_array, float *float_array)
{
    int num = 0;

    if (float_array == NULL)
    {
        LOG_E("float array is null");
        return -RT_ERROR;
    }
...
int aupipe_find_property(char *src_param, char *obj_param, int type, void *args)
{
    char *ptr_string = NULL, *temp_string = NULL;
    char *intput_string = NULL, *output_string = NULL;

    char *str_string = rt_malloc(strlen(src_param) + sizeof(char) + 4);
    char *obj_string = rt_malloc(strlen(obj_param) + sizeof(char) + 4);

    snprintf(str_string, (strlen(src_param)
             + sizeof(char) + 4) * sizeof(char), "|%s", src_param);
    snprintf(obj_string, (strlen(obj_param)
             + sizeof(char) + 4) * sizeof(char), "|%s=", obj_param);

    temp_string = strstr(str_string, obj_string);
    if (temp_string == NULL)
    {
        rt_free(str_string);
        rt_free(obj_string);
    ...
    ptr_string = strtok_r(temp_string, "|", &output_string);
    if (ptr_string == NULL)
    {
        rt_free(str_string);
        rt_free(obj_string);
    ...
    {
        if (strtok_r(ptr_string, "=", &intput_string) == NULL)
        {
            rt_free(str_string);
            rt_free(obj_string);
        ...
        else
        {
            switch(type)
            {
                ...
                {
                    int *val = (int *)args;
                    *val = atoi(intput_string);
                    break;
                }
                case VALUE_TYPE_FLOAT:
                {
                    float *val = (float *)args;
                    *val = atof(intput_string);
                    break;
                }
                case VALUE_TYPE_STRING:
                {
                    char **val = (char **)args;
                    *val = strdup(intput_string);
                    break;
                }
                default:
                {
