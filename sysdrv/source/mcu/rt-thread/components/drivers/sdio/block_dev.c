...
rt_int32_t rt_mmcsd_blk_probe(struct rt_mmcsd_card *card)
{
    ...
    if (status == RT_EOK)
    {
        for (i = 0; i < RT_MMCSD_MAX_PARTITION; i++)
        {
            ...
            status = dfs_filesystem_get_partition(&blk_dev->part, sector, 8, i);
            if (status == RT_EOK)
            {
                ...
                if (blk_dev->part.offset < 0 || blk_dev->part.size > (card->card_capacity * 2)) {
                    blk_dev->part.offset = 0;
                    blk_dev->part.size = card->card_capacity * 2;
                }
