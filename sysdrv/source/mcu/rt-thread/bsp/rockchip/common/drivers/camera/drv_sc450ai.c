/**
  ...
  * 2024-07-23     Chad Ma      fit to rv1103b
  * 2024-07-23     Chad Ma      add 4lane 1344x760@120 setting
  * 2024-09-23     Chad Ma      add 4lane 2688x1560@30 setting
  *
  ******************************************************************************
  */
...
struct sc450ai_dev
{
    ...
    const struct sc450ai_mode *pdst_mode;
 ...
 * binning to 1344x760
 */
static const uint8_t g_sc450ai_1344x760_2lane_120fps_reg_table[] =
{
    0x3, 0x01, 0x03, 0x01,
    0x3, 0x01, 0x00, 0x00,
    0x3, 0x36, 0xe9, 0x80,
 ...
 * binning to 1344x760
 */
static const uint8_t g_sc450ai_1344x760_4lane_120fps_reg_table[] =
{
    0x3, 0x01, 0x03, 0x01,
    0x3, 0x01, 0x00, 0x00,
    0x3, 0x36, 0xe9, 0x80,
    0x3, 0x36, 0xf9, 0x80,
    0x4, 0x30, 0x18, 0x7a, 0x00,
    0x3, 0x30, 0x1c, 0x78,
    0x3, 0x30, 0x1f, 0x95,
    0x4, 0x30, 0x2d, 0xa0, 0x00,
    0xa, 0x32, 0x08, 0x05, 0x40, 0x02, 0xf8, 0x03, 0xa8, 0x03, 0x0c,
    0x3, 0x32, 0x11, 0x04,
    0x5, 0x32, 0x13, 0x04, 0x11, 0x31,
    0x3, 0x32, 0x20, 0x01,
 ...
 * full resolution
 */
static const uint8_t g_sc450ai_2688x1520_4lane_60fps_reg_table[] =
{
    0x3, 0x01, 0x03, 0x01,
    0x3, 0x01, 0x00, 0x00,
    0x3, 0x36, 0xe9, 0x80,
    ...
    0x3, 0x30, 0x1f, 0x02,
    0x3, 0x30, 0x2e, 0x00,
    0xa, 0x32, 0x08, 0x0a, 0x80, 0x05, 0xf0, 0x03, 0xa8, 0x06, 0x18,
 ...
 * full resolution
 */
static const uint8_t g_sc450ai_2688x1520_4lane_30fps_reg_table[] =
{
    0x3, 0x01, 0x03, 0x01,
    0x3, 0x01, 0x00, 0x00,
    0x3, 0x36, 0xe9, 0x80,
 ...
 * binning to 1344x760
 */
static const struct sc450ai_sensor_reg g_sc450ai_1344x760_2lane_120fps_reg_table[] =
{
    {0x0103, 0x01},
    {0x0100, 0x00},
    {0x36e9, 0x80},
 ...
 * binning to 1344x760
 */
static const struct sc450ai_sensor_reg g_sc450ai_1344x760_4lane_120fps_reg_table[] =
{
    {0x0103, 0x01},
    {0x0100, 0x00},
    {0x36e9, 0x80},
 ...
 * full resolution
 */
static const struct sc450ai_sensor_reg g_sc450ai_2688x1520_4lane_60fps_reg_table[] =
{
    {0x0103, 0x01},
    {0x0100, 0x00},
    {0x36e9, 0x80},
 ...
 * full resolution
 */
static const struct sc450ai_sensor_reg g_sc450ai_2688x1520_4lane_30fps_reg_table[] =
{
    {0x0103, 0x01},
    {0x0100, 0x00},
    {0x36e9, 0x80},
...
{
    {
        ...
        .xvclk = 27000000,
        .link_freq = 360000000,
        .reg_list = g_sc450ai_1344x760_4lane_120fps_reg_table,
        .reg_list_size = ARRAY_SIZE(g_sc450ai_1344x760_4lane_120fps_reg_table),
        .hdr_mode = NO_HDR,
    },
    {
        .bus_fmt = MEDIA_BUS_FMT_SBGGR10_1X10,
        ...
        .xvclk = 27000000,
        .link_freq = 360000000,
        .reg_list = g_sc450ai_2688x1520_4lane_60fps_reg_table,
        .reg_list_size = ARRAY_SIZE(g_sc450ai_2688x1520_4lane_60fps_reg_table),
        .hdr_mode = NO_HDR,
    },
    {
        .bus_fmt = MEDIA_BUS_FMT_SBGGR10_1X10,
        ...
        .xvclk = 27000000,
        .link_freq = 360000000,
        .reg_list = g_sc450ai_2688x1520_4lane_30fps_reg_table,
        .reg_list_size = ARRAY_SIZE(g_sc450ai_2688x1520_4lane_30fps_reg_table),
        .hdr_mode = NO_HDR,
    },
};
...
{
    {
        ...
        .xvclk = 27000000,
        .link_freq = 360000000,
        .reg_list = g_sc450ai_1344x760_2lane_120fps_reg_table,
        .reg_list_size = ARRAY_SIZE(g_sc450ai_1344x760_2lane_120fps_reg_table),
        .hdr_mode = NO_HDR,
    },
    {
        .bus_fmt = MEDIA_BUS_FMT_SBGGR10_1X10,
...
const struct sc450ai_mode *get_sensor_mode(uint32_t width, uint32_t height,
        uint32_t fmt, uint32_t maxfps, uint32_t hdr_mode)
{
    int i;
    const struct sc450ai_mode *mode;

    for (i = 0; i < g_sc450ai.modes_num; i++)
    {
        mode = &g_sc450ai.supported_modes[i];
        if ((width == mode->width) &&
                (height == mode->height) &&
                (fmt == mode->bus_fmt) &&
                (hdr_mode == mode->hdr_mode) &&
                (maxfps == (mode->max_fps.denominator / mode->max_fps.numerator)))
        {
            sc450ai_info("find sensor mode id = %d, w = %d, h = %d, fmt = %d, hdr_mode = %d, maxfps = %d\n",
                         i, width, height, fmt, hdr_mode, maxfps);
            return mode;
        }
    }
    sc450ai_err("not find sensor mode w = %d, h = %d, fmt = %d, hdr_mode = %d, maxfps = %d\n",
                width, height, fmt, hdr_mode, maxfps);
    return NULL;
}
...
static void rk_continuous_reg_dump(struct sc450ai_dev *dev)
{
    ...
    for (id = 0; id < g_sc450ai.modes_num; id++)
    {
        sc450ai_info("id: %d, width: %d, height: %d\n",
                     id, g_sc450ai.supported_modes[id].width, g_sc450ai.supported_modes[id].height);
        sc450ai_write_multiple_reg((struct sc450ai_dev *)dev,
                                   g_sc450ai.supported_modes[id].reg_list,
                                   g_sc450ai.supported_modes[id].reg_list_size);
    }
}
#endif
...
static ret_err_t rk_sc450ai_set_intput_fmt(struct sc450ai_dev *dev, struct rk_camera_mbus_framefmt *mbus_fmt)
{
    ret_err_t ret = RET_SYS_EOK;
    const struct sc450ai_mode *mode;
    struct rk_camera_device *camera  = NULL;

    RT_ASSERT(dev != RT_NULL);

    camera = (struct rk_camera_device *)&dev->parent;
    sc450ai_info("dst resulotion, width %d, height %d, max_fps = %d\n",
                 dev->pdst_mode->width, dev->pdst_mode->height, dev->pdst_mode->max_fps.denominator / dev->pdst_mode->max_fps.numerator);
    if ((mbus_fmt->width == dev->pdst_mode->width) &&
            (mbus_fmt->height == dev->pdst_mode->height) &&
            (mbus_fmt->maxfps == (dev->pdst_mode->max_fps.denominator / dev->pdst_mode->max_fps.numerator)))
    {
        mode = dev->pdst_mode;
    }
    else
    {
        mode = get_sensor_mode(mbus_fmt->width, mbus_fmt->height, dev->pdst_mode->bus_fmt, mbus_fmt->maxfps, camera->info.hdr_mode);
        if (mode == NULL)
            mode = &g_sc450ai.supported_modes[1];
    }
    dev->cur_mode = mode;
    rt_mutex_take(&dev->mutex_lock, RT_WAITING_FOREVER);
...
static ret_err_t rk_sc450ai_match_dst_config(struct sc450ai_dev *dev, struct rk_camera_dst_config *dst_config)
{
    ...
    mode = get_sensor_mode(dst_config->width, dst_config->height, dev->pdst_mode->bus_fmt, dst_config->maxfps, dst_config->cam_hdr);
    if (!mode)
    {
        mode = &g_sc450ai.supported_modes[1];
    }
    memcpy(&dev->dst_mode, mode, sizeof(const struct sc450ai_mode));
    dev->pdst_mode = mode;
...
static void sc450ai_quick_stream(struct sc450ai_dev *dev, int on)
{
    sc450ai_dbg("quick stream enter tick:%u\n", rt_tick_get());
    ...
    rt_mutex_release(&dev->mutex_lock);
    dev->streaming = true;
    sc450ai_dbg("quick stream exit tick:%u\n", rt_tick_get());
}
                             ...
                             void *args)
{
    ...
    {
        sc450ai_quick_stream(sc450ai, *(int *)args);
    }
    break;
    default:
...
int rk_camera_sc450ai_init(void)
{
    ...
    if (sc450ai->lanes == 4)
    {
        instance->supported_modes = supported_modes_4lane;
        instance->modes_num = ARRAY_SIZE(supported_modes_4lane);

        sc450ai_info("using sc450ai 4 lane setting modes\n");
    }
    else if (sc450ai->lanes == 2)
    {
        instance->supported_modes = supported_modes_2lane;
        instance->modes_num = ARRAY_SIZE(supported_modes_2lane);
        sc450ai_info("using sc450ai 2 lane setting modes\n");
    }
    instance->cur_mode = get_sensor_mode(sc450ai->width, sc450ai->height,
                                         sc450ai->pixelcode, sc450ai->maxfps, sc450ai->hdr_mode);
    if (instance->cur_mode == NULL)
        instance->cur_mode = &instance->supported_modes[0];
    instance->pdst_mode = get_sensor_mode(sc450ai->dst_width, sc450ai->dst_height,
                                          sc450ai->dst_pixelcode, sc450ai->dst_maxfps, sc450ai->dst_hdr_mode);
    if (instance->pdst_mode == NULL)
        instance->pdst_mode = &instance->supported_modes[1];
    memcpy(&instance->dst_mode, instance->pdst_mode, sizeof(const struct sc450ai_mode));
    ...
    if (sc450ai->rst_gpio.gpio_group)
    {
        HAL_GPIO_SetPinDirection((struct GPIO_REG *)sc450ai->rst_gpio.gpio_group,
                                 sc450ai->rst_gpio.gpio_pin,
                                 GPIO_OUT);
        HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc450ai->rst_gpio.gpio_group,
                             sc450ai->rst_gpio.gpio_pin,
                             sc450ai->rst_active);
    }
    ...
    {
        HAL_GPIO_SetPinDirection((struct GPIO_REG *)sc450ai->pwdn_gpio.gpio_group,
                                 sc450ai->pwdn_gpio.gpio_pin,
                                 GPIO_OUT);
        HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc450ai->pwdn_gpio.gpio_group,
                             sc450ai->pwdn_gpio.gpio_pin,
                             sc450ai->pwdn_active);
    }
