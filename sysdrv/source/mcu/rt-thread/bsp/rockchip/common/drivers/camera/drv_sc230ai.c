...
static ret_err_t sc230ai_read_reg(struct rt_i2c_bus_device *bus,
                                  uint16_t reg, uint8_t *data)
{
    ...
    msg[0].addr = g_sc230ai.i2c_addr;
    msg[0].flags = RT_I2C_WR;
    msg[0].len = 2;
    msg[0].buf = send_buf;

    msg[1].addr = g_sc230ai.i2c_addr;
...
static ret_err_t sc230ai_write_reg(struct rt_i2c_bus_device *bus,
                                   uint16_t reg, uint8_t data)
{
    uint8_t send_buf[3];
...
#else
static ret_err_t sc230ai_write_reg_discontinued(struct rt_i2c_bus_device *bus,
    ...
    for (i = 0;; i++)
    {
        if (reg[i].reg_addr == REG_END)
        {
            if (k > 0)
            {
                ...
                rt_kprintf("\n");
                sc230ai_write_reg_discontinued(i2c_bus, data, k + 2);
                k = 0;
            }
            break;
        }
...
static ret_err_t rk_sc230ai_set_vts(struct sc230ai_dev *dev, uint32_t dst_vts)
{
    ...
    ret = sc230ai_write_reg(dev->i2c_bus, SC230AI_REG_VTS_L,
                            (uint8_t)(dst_vts & 0xff));
    ret |= sc230ai_write_reg(dev->i2c_bus, SC230AI_REG_VTS_H,
                             (uint8_t)(dst_vts >> 8));
...
static void sc230ai_stream_on(struct sc230ai_dev *dev)
{
    ...
    sc230ai_write_reg(dev->i2c_bus, SC230AI_REG_STREAM_CTRL, SC230AI_STREAM_ON);
    dev->streaming = true;
#endif
...
static void sc230ai_stream_off(struct sc230ai_dev *dev)
{
    ...
    if (i2c_bus)
    {
        ...
        sc230ai_write_reg(dev->i2c_bus, SC230AI_REG_STREAM_CTRL, SC230AI_STREAM_OFF);
...
int rk_camera_sc230ai_init(void)
{
    ...
    HAL_GPIO_SetPinDirection((struct GPIO_REG *)sc230ai->rst_gpio.gpio_group,
                             sc230ai->rst_gpio.gpio_pin,
                             GPIO_OUT);
    HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc230ai->rst_gpio.gpio_group,
                         sc230ai->rst_gpio.gpio_pin,
                         sc230ai->rst_active);
