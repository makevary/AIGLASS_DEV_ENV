/**
  ...
  *
  * Change Logs:
  * 2023-09-13     Chad.Ma      first implementation
  * 2024-07-19     Chad.Ma      modify reset/pwdn gpio active
  *
  ******************************************************************************
  */
...
int rk_camera_sc401ai_init(void)
{
    ...
    if (sc401ai->rst_gpio.gpio_group)
    {
        HAL_GPIO_SetPinDirection((struct GPIO_REG *)sc401ai->rst_gpio.gpio_group,
                                 sc401ai->rst_gpio.gpio_pin,
                                 GPIO_OUT);
        HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc401ai->rst_gpio.gpio_group,
                             sc401ai->rst_gpio.gpio_pin,
                             sc401ai->rst_active);
    }

    if (sc401ai->pwdn_gpio.gpio_group)
    {
        HAL_GPIO_SetPinDirection((struct GPIO_REG *)sc401ai->pwdn_gpio.gpio_group,
                                 sc401ai->pwdn_gpio.gpio_pin,
                                 GPIO_OUT);
        HAL_GPIO_SetPinLevel((struct GPIO_REG *)sc401ai->pwdn_gpio.gpio_group,
                             sc401ai->pwdn_gpio.gpio_pin,
                             sc401ai->pwdn_active);
    }
