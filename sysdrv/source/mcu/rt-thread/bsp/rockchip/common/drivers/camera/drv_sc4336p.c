...
int rk_camera_sc4336p_init(void)
{
    ...
    for (sc4336p_desc = camera_sc4336p; sc4336p_desc->i2c_addr != 0; sc4336p_desc++)
    {
        ...
        if (sc4336p_desc->mclk_id && sc4336p_desc->mclk_out_gate_id)
        {
            clk_set_rate(sc4336p_desc->mclk_id, sc4336p->cur_mode->xvclk);
            clkrate = get_clk_gate_from_id(sc4336p_desc->mclk_id);
            clk_enable(clkrate);
            clkgate = get_clk_gate_from_id(sc4336p_desc->mclk_out_gate_id);
            clk_enable(clkgate);
        }
