...
#ifdef SOC_RV1103B
#define ACLK_ISP3P2_GATE ACLK_ISP_GATE
#define HCLK_ISP3P2_GATE HCLK_ISP_GATE
#define CLK_CORE_ISP3P2_GATE CLK_CORE_ISP_GATE
#define PCLK_VICAP_GATE PCLK_CRU_VI_GATE
#define DCLK_VICAP_GATE NULL
#define PCLK_VICAP_VEPU_GATE PCLK_CRU_VEPU_GATE
#define I1CLK_VICAP_GATE NULL
#define I0CLK_VICAP_GATE NULL
#define RX1PCLK_VICAP_GATE NULL
#define RX0PCLK_VICAP_GATE NULL
#define PCLK_CSIHOST0_GATE PCLK_CSI2HOST0_GATE
#define PCLK_CSIHOST1_GATE PCLK_CSI2HOST1_GATE
#define PCLK_MIPICSIPHY_GATE PCLK_CSIPHY_GATE
#define CLK_RXBYTECLKHS_0_GATE NULL
#define CLK_RXBYTECLKHS_1_GATE NULL
#endif
                                   ...
                                   uint32_t align)
{
    ...
    ISP_INFO(dev, "buf head[0x%x] heap->end_addr 0x%x. buf size %d\n", mem, heap->end_addr, size);
...
#if ISP_DEBUG
static void rk_isp_dump_vicap_reg(struct rk_isp_dev *dev)
{
    uint32_t vicap_base;
    ...
    read_start = vicap_base + 0x100;
    ISP_INFO(dev, "0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
             vicap_base, RKISP_READ(vicap_base), RKISP_READ(vicap_base + 4),
             RKISP_READ(vicap_base + 8), RKISP_READ(vicap_base + 12));
    for (i = 0; i < (0x100 / 16); i++)
    {
        ISP_INFO(dev, "0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
                 read_start + i * 16, RKISP_READ(read_start + i * 16), RKISP_READ(read_start + i * 16 + 4),
                 RKISP_READ(read_start + i * 16 + 8), RKISP_READ(read_start + i * 16 + 12));
    }
    ISP_INFO(dev, "0x%08x: 0x%08x 0x%08x 0x%08x 0x%08x\n",
             vicap_base + TOISP0_CH_CTRL, RKISP_READ(vicap_base + TOISP0_CH_CTRL), RKISP_READ(vicap_base + TOISP0_CH_CTRL + 4),
             RKISP_READ(vicap_base + TOISP0_CH_CTRL + 8), RKISP_READ(vicap_base + TOISP0_CH_CTRL + 12));
}
#endif
...
static rt_err_t rk_isp_hw_set_stream_on(struct rk_isp_dev *dev)
{
    ...
    ISP_DBG("crop %d, width %d, height %d, format %d\n",
            output_info->is_crop,
            width,
            height,
            output_info->output_fmt->fourcc);
    /* start to config isp */
    ...
    if (output_info->output_fmt->fourcc == V4L2_PIX_FMT_NV16 ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_NV61 ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_NV12 ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_NV21 ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_YUYV ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_YVYU ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_UYVY ||
            output_info->output_fmt->fourcc == V4L2_PIX_FMT_VYUY)
    {
                                   ...
                                   struct isp_video_requestbuffers *req)
{
    ...
    if (req->type == ISP_BUF_MEM_TYPE_COMPACT)
    {
        ...
        if (isp_output_format->fmt_type == ISP_DEVICE_FORMAT_RAW)
        {
            ...
            size_per_buf = height * bytesperline; //There is needed more than 24 lines to fix isp controler bug
...
static void rk_irq_handler_vicap(int vector, void *param)
{
    ...
    if (csi_host0->enable)
    {
        intstat = RKISP_READ(vicap_base + CSI_MIPI0_INTSTAT);
        if (intstat)
        {
            RKISP_WRITE(vicap_base + CSI_MIPI0_INTSTAT, intstat);
            ISP_DBG(ispdev, "intstat 0x%x\n", intstat);
        }

        if (intstat & VICAP_MIPI0_OVERFLOW_Y)
        {
            ISP_INFO(ispdev, "ERR: vicap overflow y, 0x%x\n", intstat);
        }
        if (intstat & VICAP_MIPI0_BANDWITHLACK)
        {
            ISP_INFO(ispdev, "ERR: vicap bandwith lack, 0x%x\n", intstat);
        }
        if (intstat & VICAP_MIPI0_OVERFLOW_CSI2)
        {
            ISP_INFO(ispdev, "ERR: vicap overflow csi2, 0x%x\n", intstat);
        }
        if (intstat & VICAP_MIPI0_ID0_SIZE_ERR)
        {
            ISP_INFO(ispdev, "ERR: vicap id0 size err, 0x%x, tick: %d\n", intstat, rt_tick_get());
        }
        if (intstat & VICAP_MIPI0_ID0_FRM_END)
        {
            buf = rk_isp_hw_vicap_update_buf(ispdev, 0, (intstat & VICAP_MIPI0_ID0_FRM_END) >> VICAP_MIPI0_ID0_FRM_END_SHIFT);
            if (buf)
            {
        ...
        if (intstat & VICAP_MIPI0_ID0_FRM_START)
        {
            ispdev->fs_timestamp = rt_tick_get();
            ISP_DBG(ispdev, "vicap id0 fs %d\n", ispdev->fs_timestamp);
        }
    }
    if (csi_host1->enable)
    {
        intstat = RKISP_READ(vicap_base + CSI_MIPI1_INTSTAT);
        if (intstat)
        {
            RKISP_WRITE(vicap_base + CSI_MIPI1_INTSTAT, intstat);
            ISP_DBG(ispdev, "intstat 0x%x\n", intstat);
        }
        if (intstat & VICAP_MIPI0_OVERFLOW_Y)
        {
        ...
        if (intstat & VICAP_MIPI0_ID0_SIZE_ERR)
        {
            ISP_INFO(ispdev, "ERR: vicap id0 size err, 0x%x\n", intstat);
        }
        if (intstat & VICAP_MIPI0_ID0_FRM_END)
        {
            buf = rk_isp_hw_vicap_update_buf(ispdev, 0x200, (intstat & VICAP_MIPI0_ID0_FRM_END) >> VICAP_MIPI0_ID0_FRM_END_SHIFT);
            if (buf)
            {
                buf->timestamp = ispdev->fs_timestamp;
                rk_isp_videobuf_done(buf, ISP_BUF_STATE_DONE, ISP_BUF_LIST_TYPE_MP);
...
rt_err_t rk_isp_init(struct rk_isp_device *dev)
{
    ...
    // 0x3000 * CAM_NUM_2
    size = SHARED_MEM_RESERVED_HEAD_SIZE * 2;
#if !defined(SOC_RV1106)
    ispdev->w3a_mem_addr = isp_mem_config->share_mem_addr + size;
    ispdev->w3a_mem_size = SHARED_MEM_RESERVED_HEAD_SIZE;
    //0x3000 * (CAM_NUM_2 + W3A)
    size += SHARED_MEM_RESERVED_HEAD_SIZE;
#endif
