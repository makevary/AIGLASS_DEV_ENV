...
static struct sc200ai_mode supported_modes[] =
{
    ...
    [SC200AI_1920X1080_60FPS] = {
        ...
        .reg_list_size = ARRAY_SIZE(sc200ai_hdr_1920x1080_60fps_regs),
        .hdr_mode = NO_HDR,
    },
    [SC200AI_1920X1080] = {
        .bus_fmt = MEDIA_BUS_FMT_SBGGR10_1X10,
...
static int sc200ai_set_gain_reg(struct sc200ai *dev, uint32_t gain, int mode)
{
    ...
    {
        ret = sc200ai_write_reg(dev, SC200AI_REG_SDIG_GAIN,
                                DIG_gain_reg & 0xF);
        ret |= sc200ai_write_reg(dev, SC200AI_REG_SDIG_FINE_GAIN,
                                 DIG_Fine_gain_reg);
        ret |= sc200ai_write_reg(dev, SC200AI_REG_SANA_GAIN,
                                 Coarse_gain_reg);
        ret |= sc200ai_write_reg(dev, SC200AI_REG_SANA_FINE_GAIN,
                                 ANA_Fine_gain_reg);
    }
...
{
    sc200ai_dbg(dev, "quick stream enter tick:%u\n", rt_tick_get());
    ...
    rt_mutex_release(&dev->mutex_lock);
    dev->streaming = true;
    sc200ai_dbg(dev, "quick stream exit tick:%u\n", rt_tick_get());
}
...
ret_err_t rk_sc200ai_init(struct rk_camera_device *dev)
{
    ...
    if (sc200ai)
    {
        ...
        camera->info.mbus_fmt.pixelcode = sc200ai->cur_mode->bus_fmt;//0x0c uyvy;0x08 vyuy;0x04 yvyu;0x00 yuyv
...
static ret_err_t rk_sc200ai_match_dst_config(struct sc200ai *dev, struct rk_camera_dst_config *dst_config)
{
    ...
    sc200ai_info(dev, "cur resolution, width %d, height %d, fps %d!\n",
    ...
    for (i = 0; i < ARRAY_SIZE(supported_modes); i++)
    {
                ...
                dst_config->height == mode->height)
        {
            ...
            ret = RT_EOK;
            sc200ai_info(dev, "find dst resolution, width %d, height %d, fps %d!\n",
                         dst_config->width, dst_config->height,
                         dev->dst_mode->max_fps.denominator / dev->dst_mode->max_fps.numerator);
            break;
        }
    }
    if (i == ARRAY_SIZE(supported_modes))
    {
        sc200ai_err(dev, "not find match resolution\n");
        return -RT_EINVAL;
    }
...
static ret_err_t rk_sc200ai_set_intput_fmt(struct sc200ai *dev, struct rk_camera_mbus_framefmt *mbus_fmt)
{
    ...
    sc200ai_info(dev, "set format: width %d, height %d\n",
                             ...
                             void *args)
{
    ...
    break;
    case RK_DEVICE_CTRL_CAMERA_QUICK_STREAM:
    {
