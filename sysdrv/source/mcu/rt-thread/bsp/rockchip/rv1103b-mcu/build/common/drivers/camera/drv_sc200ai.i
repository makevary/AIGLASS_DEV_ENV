# 1 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
# 1 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/rv1103b-mcu//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
# 15 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
...
# 16 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 2
...
# 22 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 2





__attribute__((weak)) const struct camera_board_desc camera_sc200ai[] = {0};
# 107 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
...
# 146 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
   _Bool 
# 146 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
        has_init_exp;
    
# 147 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
   _Bool 
# 147 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
        streaming;
};
#pragma pack()
...
static const uint8_t sc200ai_hdr_1920x1080_60fps_regs[] =
{
    0x3, 0x01, 0x03, 0x01,
    0x3, 0x01, 0x00, 0x00,
    0x3, 0x36, 0xe9, 0x80,
...
static struct sc200ai_mode supported_modes[] =
{
    [SC200AI_960X540] = {
        ...
        .reg_list_size = (sizeof(sc200ai_linear_960x540_120fps_regs) / sizeof((sc200ai_linear_960x540_120fps_regs)[0])),
        .hdr_mode = NO_HDR,
    },
    [SC200AI_1920X1080_60FPS] = {
        .bus_fmt = 0x3007,
        ...
        .xvclk = 27000000,
        .link_freq = 371250000,
        .reg_list = sc200ai_hdr_1920x1080_60fps_regs,
        .reg_list_size = (sizeof(sc200ai_hdr_1920x1080_60fps_regs) / sizeof((sc200ai_hdr_1920x1080_60fps_regs)[0])),
        .hdr_mode = NO_HDR,
    },
    [SC200AI_1920X1080] = {
        .bus_fmt = 0x3007,
        ...
        .reg_list_size = (sizeof(sc200ai_linear_1920x1080_30fps_regs) / sizeof((sc200ai_linear_1920x1080_30fps_regs)[0])),
        .hdr_mode = NO_HDR,
    },
    [SC200AI_1920X1080_HDR] = {
        .bus_fmt = 0x3007,
        ...
        .reg_list_size = (sizeof(sc200ai_hdr_1920x1080_30fps_regs) / sizeof((sc200ai_hdr_1920x1080_30fps_regs)[0])),
        .hdr_mode = HDR_X2,
    }
};
...
static ret_err_t sc200ai_read_reg(struct sc200ai *dev, uint16_t reg, uint8_t *data)
{
    ...
    while (1)
    {
        ...
        {
            rt_kprintf("sc200ai-%s error ""read reg, retry=%d, reg [0x%x]\n""", dev->name, retry, reg);
            if (--retry)
                continue;
            else
                return (-1);
...
static ret_err_t sc200ai_write_reg(struct sc200ai *dev, uint16_t reg, uint8_t data)
{
    ...
    while (1)
    {
        ...
        {
            rt_kprintf("sc200ai-%s error ""write reg, retry:%d, reg [0x%x]-->0x%x\n""", dev->name, retry, reg, data);
            if (--retry)
                continue;
            else
                return (-1);
...
static int sc200ai_write_multiple_reg_continue(struct sc200ai *dev, const uint8_t *i2c_data, int len)
{
    ...
    for (i = 0; i < i2c_data[len - 1];)
    {
        ...
        if (ret != 1)
        {
            rt_kprintf("sc200ai-%s error ""write multi-regs, retry=%d, addr=0x%02x%02x\n""", dev->name, retry, i2c_data[offset + 1], i2c_data[offset + 2])
                                                                          ;
            if (--retry)
                continue;
            else
...
static int sc200ai_set_gain_reg(struct sc200ai *dev, uint32_t gain, int mode)
{
    ...
    if (mode == 0)
    {
        ret = sc200ai_write_reg(dev, 0x3e06,
                                DIG_gain_reg & 0xF);
        ret |= sc200ai_write_reg(dev, 0x3e07,
                                 DIG_Fine_gain_reg);
        ret |= sc200ai_write_reg(dev, 0x3e08,
                                 Coarse_gain_reg);
        ret |= sc200ai_write_reg(dev, 0x3e09,
                                 ANA_Fine_gain_reg);
    }
    else
    {
        ret = sc200ai_write_reg(dev, 0x3e10,
                                DIG_gain_reg & 0xF);
        ret |= sc200ai_write_reg(dev, 0x3e11,
                                 DIG_Fine_gain_reg);
        ret |= sc200ai_write_reg(dev, 0x3e12,
                                 Coarse_gain_reg);
        ret |= sc200ai_write_reg(dev, 0x3e13,
                                 ANA_Fine_gain_reg);
    }
...
static rt_err_t rk_sc200ai_set_expval(struct sc200ai *dev, struct rk_camera_exp_val *exp)
{
    ...
    rt_mutex_take(&dev->mutex_lock, -1);

    if (!dev->has_init_exp && !dev->streaming && strcmp(sc200ai->isp_subdev_name, "sensor_1"))
    {
        dev->init_exp = *exp;
        dev->has_init_exp = 
# 795 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                           1
# 795 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
                               ;
        ;
    ...
    ret = sc200ai_read_reg(dev, 0x3e23, &s_max_exp_time_h);
    ret = sc200ai_read_reg(dev, 0x3e24, &s_max_exp_time_l);
    ...
    ret = sc200ai_write_reg(dev, 0x3e00,
                            (((l_exp_time) >> 12) & 0xF));
    ret = sc200ai_write_reg(dev, 0x3e01,
                            (((l_exp_time) >> 4) & 0xFF));
    ret = sc200ai_write_reg(dev, 0x3e02,
                            (((l_exp_time) & 0xF) << 4));
    if (dev->cur_mode->hdr_mode == HDR_X2)
    {
        ret = sc200ai_write_reg(dev, 0x3e04,
                                (((s_exp_time) >> 4) & 0xFF));
        ret = sc200ai_write_reg(dev, 0x3e05,
                                (((s_exp_time) & 0xF) << 4));
    }

    ret = sc200ai_set_gain_reg(dev, l_a_gain, 0);
    if (dev->cur_mode->hdr_mode == HDR_X2)
        ret |= sc200ai_set_gain_reg(dev, s_a_gain, 1);
...
static ret_err_t rk_sc200ai_set_vts(struct sc200ai *dev, uint32_t dst_vts)
{
    ...
    ret = sc200ai_write_reg(dev, 0x320f,
                            (uint8_t)(dst_vts & 0xff));
    ret = sc200ai_write_reg(dev, 0x320e,
                            (uint8_t)(dst_vts >> 8));
...
static ret_err_t rk_sc200ai_set_flip_mirror(struct sc200ai *dev, uint32_t flip)
{
    ...
    ret = sc200ai_read_reg(dev, 0x3221, &val);
    switch (flip)
    {
    case 0 ... 3:
        val = (flip & 0x01 ? val | 0x06 : val & 0xf9);
        val = ((flip >> 1) & 0x01 ? val | 0x60 : val & 0x9f);
        break;
    default:
        val = (
# 888 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
             0 
# 888 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
             ? val | 0x06 : val & 0xf9);
        val = (
# 889 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
             0 
# 889 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
             ? val | 0x60 : val & 0x9f);
        break;
    };
    ;
    ret = sc200ai_write_reg(dev, 0x3221, val);
...
# 920 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
    rt_mutex_release(&dev->mutex_lock);
    ;
}
...
static void sc200ai_stream_on_late(struct sc200ai *dev)
{
    ...
    sc200ai_write_reg(dev, 0x0100, 0x01);
    rt_mutex_release(&dev->mutex_lock);
    dev->streaming = 
# 940 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                    1
# 940 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
                        ;
    ;
}

static void sc200ai_quick_stream(struct sc200ai *dev, int on)
{
    ;
    ...
    {
        sc200ai_write_reg(dev, 0x0100, 0x00);
    }
    rt_mutex_release(&dev->mutex_lock);
    dev->streaming = 
# 959 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                    1
# 959 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
                        ;
    ;
}
...
static void sc200ai_stream_off(struct sc200ai *dev)
{
    ...
    dev->streaming = 
# 976 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                    0
# 976 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
                         ;
}
ret_err_t rk_sc200ai_init(struct rk_camera_device *dev)
{
    ...
    if (sc200ai)
    {
        ...
        camera->info.mbus_fmt.pixelcode = sc200ai->cur_mode->bus_fmt;
        ...
        camera->info.mbus_config.flags = (0x1 << (0x0)) |
                                         (0x1 << (0x6));
...
static ret_err_t rk_sc200ai_match_dst_config(struct sc200ai *dev, struct rk_camera_dst_config *dst_config)
{
    ...
    rt_kprintf("sc200ai-%s ""cur resolution, width %d, height %d, fps %d!\n""", dev->name, dev->cur_mode->width, dev->cur_mode->height, dev->cur_mode->max_fps.denominator / dev->cur_mode->max_fps.numerator)

                                                                                       ;

    for (i = 0; i < (sizeof(supported_modes) / sizeof((supported_modes)[0])); i++)
    {
                ...
                dst_config->height == mode->height)
        {
            ...
            ret = 0;
            rt_kprintf("sc200ai-%s ""find dst resolution, width %d, height %d, fps %d!\n""", dev->name, dst_config->width, dst_config->height, dev->dst_mode->max_fps.denominator / dev->dst_mode->max_fps.numerator)

                                                                                               ;
            break;
        }
    }
    if (i == (sizeof(supported_modes) / sizeof((supported_modes)[0])))
    {
        rt_kprintf("sc200ai-%s error ""not find match resolution\n""", dev->name);
        return -10;
    }
    ...
    if (dst_fps > cur_fps)
    {
        rt_kprintf("sc200ai-%s error ""dst fps is larger than cur fps\n""", dev->name);
        return -10;
    }
...
# 1127 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
   _Bool 
# 1127 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
        is_find_fmt = 
# 1127 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                      0
# 1127 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
    ...
    rt_kprintf("sc200ai-%s ""set format: width %d, height %d\n""", dev->name, mbus_fmt->width, mbus_fmt->height)
                                                   ;
            ...
            mbus_fmt->height == dev->dst_mode->height)
    {
        mode = dev->dst_mode;
        is_find_fmt = 
# 1135 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                     1
# 1135 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
                         ;
    }
    else
    {
        for (i = 0; i < (sizeof(supported_modes) / sizeof((supported_modes)[0])); i++)
        {
            ...
            {
                is_find_fmt = 
# 1145 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c" 3 4
                             1
# 1145 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/common/drivers/camera/drv_sc200ai.c"
                                 ;
                break;
            }
        }
    ...
    if (is_find_fmt)
    {
        if (mode->width != dev->cur_mode->width || !strcmp(dev->name, "sensor_1"))
        {
            dev->cur_mode = mode;
            camera->info.mbus_config.linked_freq = dev->cur_mode->link_freq;
            camera->info.mbus_fmt.pixelcode = dev->cur_mode->bus_fmt;
            rt_mutex_take(&dev->mutex_lock, -1);
            sc200ai_write_multiple_reg_continue(dev, dev->cur_mode->reg_list, dev->cur_mode->reg_list_size);
                             ...
                             void *args)
{
    ...
    case (27):
    {
        sc200ai_quick_stream(sc200ai, *(int *)args);
    }
    break;
    default:
        ;
...
int rk_camera_sc200ai_init(void)
{
    ...
    for (sc200ai_desc = camera_sc200ai; sc200ai_desc->i2c_addr != 0; sc200ai_desc++)
    {
        ...
        rt_memcpy(sc200ai->name, sc200ai_desc->isp_subdev_name, (12));
        rt_memcpy(sc200ai->i2c_name, sc200ai_desc->i2c_bus, (12));

        sc200ai->i2c_bus = (struct rt_i2c_bus_device *)rt_device_find(sc200ai->i2c_name);
        if (sc200ai->i2c_bus == (0))
        {
            rt_kprintf("sc200ai-%s error ""can't find i2c bus:%s\n""", sc200ai->name, sc200ai->i2c_name);
            rt_free(sc200ai);
            continue;
        }
        ...
        {
            rt_kprintf("sc200ai-%s ""mode id is over range, default use mode_id: 0\n""", sc200ai->name);
        ...
        rt_mutex_init(&sc200ai->mutex_lock, "sc200ai_mutex", 0x00);
        ;
        camera->i2c_bus = sc200ai->i2c_bus;
        rt_memcpy(camera->name, sc200ai->name, (12));
        ret = rk_camera_register(camera, camera->name, sc200ai);
        if (ret)
        {
            rt_kprintf("sc200ai-%s error ""regster failed\n""", sc200ai->name);
            rt_free(sc200ai);
        }
    }
...
__attribute__((used)) const init_fn_t __rt_init_rk_camera_sc200ai_init __attribute__((section(".rti_fn.""3"))) = rk_camera_sc200ai_init;
