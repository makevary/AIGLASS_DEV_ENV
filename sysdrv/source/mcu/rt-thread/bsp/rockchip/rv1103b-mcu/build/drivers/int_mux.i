...
void rt_hw_interrupt_active(int vector)
{
    ...
    index = vector / 32;
    set = vector % 32;
    temp = *(unsigned int *)(0x20a20000 + index * 4);
    *(unsigned int *)(0x20a20000 + index * 4) = (1 << set) | temp;
}
...
void rt_hw_interrupt_deactive(int vector)
{
    ...
    index = vector / 32;
    set = vector % 32;
    *(unsigned int *)(0x20a20000 + index * 4) &= ~(1 << set);
}
...
rt_uint32_t rt_hw_interrupt_get_active(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (*(unsigned int *)(0x20a20000 + 0x80 + i * 4))
            break;
    }

    if (i < 4)
    {
        int_reg = *(unsigned int *)(0x20a20000 + 0x80 + i * 4);
        for (n = 0; n < 32; n++)
        {
