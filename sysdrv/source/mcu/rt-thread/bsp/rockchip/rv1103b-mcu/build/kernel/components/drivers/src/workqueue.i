# 1 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/components/drivers/src/workqueue.c"
# 1 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/bsp/rockchip/rv1103b-mcu//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/components/drivers/src/workqueue.c"
# 11 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/components/drivers/src/workqueue.c"
...
# 12 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/components/drivers/src/workqueue.c" 2
...
# 14 "/opt/aiglass_dev_env/sysdrv/source/mcu/rt-thread/components/drivers/src/workqueue.c" 2


static __inline rt_err_t _workqueue_work_completion(struct rt_workqueue *queue)
{
    rt_err_t result;
...
static void _workqueue_thread_entry(void* parameter)
{
    ...
    while (1)
    {
        ...
        work = ((struct rt_work *)((char *)(queue->work_list.next) - (unsigned long)(&((struct rt_work *)0)->list)));
...
struct rt_workqueue *rt_workqueue_create(const char* name, rt_uint16_t stack_size, rt_uint8_t priority)
{
    struct rt_workqueue *queue = (0);

    queue = (struct rt_workqueue*)rt_malloc(sizeof(struct rt_workqueue));
    if (queue != (0))
    {

        rt_list_init(&(queue->work_list));
        queue->work_current = (0);
        rt_sem_init(&(queue->sem), "wqueue", 0, 0x00);


        queue->work_thread = rt_thread_create(name, _workqueue_thread_entry, queue, stack_size, priority, 10);
        if (queue->work_thread == (0))
        {
            rt_free(queue);
            return (0);
        }
