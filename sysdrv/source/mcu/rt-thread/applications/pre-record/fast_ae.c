...
uint32_t *g_exp_gain = 123;
uint32_t *g_exp_time = 4523;
...
int32_t parse_meta_params()
{
    ...
    RT_ASSERT(i >= 0);
#endif
    rt_kprintf("wait meta end tick %u\n", rt_tick_get());
    ...
    *g_app_param = *app_param;
    ...
    g_exp_gain = &app_param->exp_gain;
    g_exp_time = &app_param->exp_time;
    // rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)0x3828000, 0x10000); 
    // rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, (void *)0x3828000, 0x10000);
    // writel(0x12345678, 0x3828000);
    // writel(0x87654321, 0x3828004);
    // uint32_t tmp_val;
    // tmp_val = readl(0x3828000);
    // rt_kprintf("-------gain %08x\n", tmp_val);
    // tmp_val = readl(0x3828004);
    // rt_kprintf("-------gain %08x\n", tmp_val);
    // rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)0x3828000, 0x10000); 
...
{
    struct rk_camera_exp_val exp_val = {0};
    AE_RESULT ret = 0;
    g_is_ie_effect = 0;
    gShare = share;
    ...
    ret = rk_device_control(sensor, RK_DEVICE_CTRL_CAMERA_GET_EXP_INIT, &exp_val);
    *g_exp_gain = exp_val.reg_gain[0];
    *g_exp_time = exp_val.reg_time[0];
