                             ...
                             bool ablc_en)
{
    ...
    else if (awb_stat_calib->hw_awbCfg_statsSrc_mode == awbStats_btnrOut_mode)
    {
        ...
        if (hdr_mode > 0)
        {
            ...
            rk_kprintf("hdrmge_gain0_1=%s\n", FixPrint((int)(hdrmge_gain0_1 * (1 << 16)), 2));
                      ...
                      int hdr_mode)
{
    ...
    rk_kprintf("fastawb: rtt_awbgain(%d,%d,%d,%d), pre_wbgain_inv_r(%d,%d,%d)\n", rtt_awbgain.r, rtt_awbgain.gr, rtt_awbgain.gb, rtt_awbgain.b,
               awb_cfg->pre_wbgain_inv_r, awb_cfg->pre_wbgain_inv_g, awb_cfg->pre_wbgain_inv_b);
    /*
        if (otp_awb != NULL && otp_awb->flag) {
            rk_kprintf("%s before otp pre_wbgain_inv = [%d, %d, %d]\n", __FUNCTION__,
...
void  ConfigLimitRange(const awb_rgbySpace_t *rgbySpace, awbStats_rgbyWp_t *wpDct_rgbySpace)
{
    ...
    {
        rk_kprintf("no rgbySpace config !!! rgbyLimit->cfg.cfg_len = %d\n", rgbySpace->cfg.cfg_len);
        return;
    }
...
int set_awb_params_for_kernel(struct isp33_isp_params_cfg *cfg, awb_api_attrib_t *wb_calib, int hdr_mode, int width, int height,
                              float isp_dgain, float isp_ob_predgain, float aec_iso, float hdrmge_gain0_1, bool ablc_en, struct isp_awbgain_cfg rtt_awbgain)
{
    ...
    //rk_kprintf("blc2_cfg %d %d,%d,%d,%d\n", awb_cfg->bls2_en, awb_cfg->bls2_val.r, awb_cfg->bls2_val.gr, awb_cfg->bls2_val.gb, awb_cfg->bls2_val.b);
    ...
    if (awb_cfg->light_num > AWBSTATS_WPDCT_LS_NUM)
    {
        ...
        rk_kprintf("fastawb: light_num should be < %d!\n", AWBSTATS_WPDCT_LS_NUM);
    }
