...
uint32_t rkcif_mbus_pixelcode_to_v4l2(uint32_t pixelcode)
{
    ...
    case MEDIA_BUS_FMT_SRGGB12_1X12:
        ...
        break;
    default:
        STREAM_ERR("unsupport pixel format %x\n", pixelcode);
        break;
    }
...
static ret_err_t stream_get_frame(rk_device *dev, bool is_sample, struct shared_data *share)
{
    ...
    STREAM_DBG("dqueue buf 0x%x tick %u!\n", buf.addr, rt_tick_get());
...
retry:
    ...
    STREAM_INFO("disable interrupt!\n");
    HAL_DCACHE_CleanInvalidate();
    STREAM_INFO("clean&invalid dache!\n");
    HAL_DCACHE_Disable();
    STREAM_INFO("disable dache!\n");
    sysTick_stop();
    STREAM_INFO("disable tick & send mabox!\n");
