...
#ifdef SMART_MEETING_UAC_ENABLE
...
#define UAC_SET_VOL_MUTE_INTERVAL 100
...
struct audio_play
{
    ...
    rt_list_t free_list;
    rt_size_t free_list_len;
    rt_list_t ready_list;
    rt_size_t ready_list_len;
...
static rt_err_t _audio_playback_card_enable(struct audio_play *p)
{
    ...
    if (rkstudio_open(SMART_MEETING_PLAYBACK_CARD,
                      p->sample_rate, P_AUDIO_SAMPLE_BITS,
                      P_NR_CHANNEL) != RT_EOK)
    {
        rt_kprintf("rkstudio open failed\n");
        return -RT_ERROR;
    }
    p->total_size = P_TOTAL_SIZE;
#ifdef RT_USB_AUDIO_PLL_COMPENSATION
...
static rt_err_t _audio_playback_card_set_volume(struct audio_play *p)
{
    ...
    if (p->cur_volume != p->set_volume && !p->cur_mute)
    {
...
static rt_err_t _audio_capture_card_enable(struct audio_capture *c)
{
    ...
    if (rkaudio_open(SMART_MEETING_CAPTURE_CARD, c->sample_rate,
                     C_AUDIO_SAMPLE_BITS, C_NR_CHANNEL) != RT_EOK)
    {
        rt_kprintf("rkaudio open failed\n");
        return -RT_ERROR;
    }
    c->send_size = 0;
...
static void playback_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (msg == MSG_START)
        {
            rt_kprintf("_audio_playback_card_enable\n");
        ...
        {
            rt_kprintf("_audio_playback_card_disable\n");
        ...
        if (timeout == RT_WAITING_FOREVER)
        {
            ...
            if (p->ready_list_len > P_BUFFER_READY_NUM)
        ...
        if (timeout == RT_WAITING_NO)
        {
            ...
            if (pbuf)
            {
                if (!rkstudio_write(pbuf->buffer, P_AUDIO_PERIOD_SIZE))
                ...
                else if (0 && (ready_len == 0 || up >= P_BUFFER_NUM - 1))
                {
                    /*
...
//        p->pbuf[i].buffer = rt_malloc(P_TOTAL_SIZE);
        p->pbuf[i].buffer = rt_malloc_align(P_TOTAL_SIZE, 128);
        if (!p->pbuf[i].buffer)
        {
...
static void capture_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (msg == MSG_START)
        {
            rt_kprintf("_audio_capture_card_enable\n");
            _audio_capture_card_enable(c);
            timeout = RT_WAITING_NO;
        }
        else if (msg == MSG_STOP)
        {
            rt_kprintf("_audio_capture_card_disable\n");
        ...
        if (timeout == RT_WAITING_NO)
        {
            ...
            if (cbuf)
            {
                if ((ret = rkaudio_read(cbuf->buffer, C_AUDIO_PERIOD_SIZE)) <= 0)
                    rt_kprintf("Error capture %d\n", ret);
...
{
    uint32_t val = READ_REG(GRF->OS_REG5);
    if (val == 0)
