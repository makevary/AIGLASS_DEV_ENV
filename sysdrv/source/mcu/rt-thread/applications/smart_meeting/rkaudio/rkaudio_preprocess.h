/**
  ...
  *
  ******************************************************************************
  * @file    rkaudio_preprocess.c
  * @author  Jair Wu
  * @version v0.1
  * @date    2023.4.20
  * @brief   The rkaudio param
  ******************************************************************************
  */
...
inline static void * rkaudio_dereverb_param_init(void *alloc){
	RKAudioDereverbParam* param = (RKAudioDereverbParam*)alloc;
...
inline static void* rkaudio_aes_param_init(void *alloc) { 
	RKAudioAESParameter* param = (RKAudioAESParameter*)alloc;
...
inline static void* rkaudio_nlp_param_init(void *alloc)
{
	SKVNLPParameter* param = (SKVNLPParameter*)alloc;
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 2; j++)
		{
...
inline static void *rkaudio_anr_param_init(void *alloc) {
	SKVANRParam* param = (SKVANRParam*)alloc;
	/* anr parameters */
...
inline static void* rkaudio_agc_param_init(void *alloc)
{
	RKAGCParam* param = (RKAGCParam*)alloc;
...
inline static void* rkaudio_cng_param_init(void *alloc)
{
	RKCNGParam* param = (RKCNGParam*) alloc;
	/* cng paremeters */
...
inline static void* rkaudio_dtd_param_init(void *alloc)
{
	RKDTDParam* param = (RKDTDParam*) alloc;
	/* dtd paremeters*/
...
inline static void* rkaudio_aec_param_init(void *alloc)
{
	SKVAECParameter* param = (SKVAECParameter*) alloc;
...
inline static void * rkaudio_eq_param_init(void *alloc){
	RKaudioEqParam* param = (RKaudioEqParam*) alloc;
	param->shwParaLen = 65;
	int i, j;
	for (i = 0; i < 5; i++){
		for (j = 0; j < 13; j++) {
...
inline static void* rkaudio_preprocess_param_init(void *alloc)
{
	SKVPreprocessParam* param = (SKVPreprocessParam*) alloc;
	//param->model_bf_en = EN_Anr;
	//param->model_bf_en = EN_Agc | EN_Anr | EN_Dereverberation;
	//param->model_bf_en = EN_Fastaec | EN_AES | EN_Anr | EN_Agc | EN_STDT | EN_Dereverberation;
	...
	param->drop_ref_channel = NUM_DROP_CHANNEL;
//	param->dereverb_param = rkaudio_dereverb_param_init();
//	param->aes_param = rkaudio_aes_param_init();
//	param->nlp_param = rkaudio_nlp_param_init();
//	param->anr_param = rkaudio_anr_param_init();
//	param->agc_param = rkaudio_agc_param_init();
//	param->cng_param = rkaudio_cng_param_init();
//	param->dtd_param = rkaudio_dtd_param_init();
//	param->eq_param = rkaudio_eq_param_init();
	return (void*)param;
}
/* Set the Sub-Para which used to initialize the RX*/
inline static void* rkaudio_rx_param_init(void *alloc)
{
	RkaudioRxParam* param = (RkaudioRxParam*)alloc;
//	param->anr_param = rkaudio_anr_param_init();
...
inline static void rkaudio_preprocess_param_destory(void* param_)
{
	...
	free(param->dereverb_param); param->dereverb_param = NULL;
	free(param->aes_param); param->aes_param = NULL;
	free(param->anr_param); param->anr_param = NULL;
	free(param->agc_param); param->agc_param = NULL;
	free(param->nlp_param); param->nlp_param = NULL;
	free(param->cng_param); param->cng_param = NULL;
	free(param->dtd_param); param->dtd_param = NULL;
	free(param->eq_param); param->eq_param = NULL;
	free(param); param = NULL;
}
