...
if [ "${1}"x = "y"x ];then
	if [ -f "/oem/usr/ko/dw_mmc.ko" ]; then
		insmod dw_mmc.ko
	...
	case "$2" in
		...
		HI3861L)
			...
			insmod /oem/usr/ko/hichannel.ko hi_rk_irq_gpio=68
			;;
		*)
			exit 1
			;;
...
#SSV6115P and SSV6115C
cat /sys/bus/usb/devices/*/uevent | grep "8065\/6011\/100"
if [ $? -eq 0 ];then
	echo "WIFI CHIP IS SSV6355"
	insmod cfg80211.ko && insmod ssv6355.ko
	break
fi
