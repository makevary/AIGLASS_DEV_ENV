...
struct ssv6xxx_cfg {
    ...
    // extend connect timeout (unit: ms) 
    u32  auth_timeout;
    u32  asso_timeout;
    // sta detect bcn timeout (unit: ms) 
    u32 sta_detect_bcn_timeout;

    // update data rate frequency
    u32 data_rate_update_freq;

	//specific ap 
    u32  specific_ap;

    //force txrate sgi off
    u8 txrate_force_sgi_off;
};
