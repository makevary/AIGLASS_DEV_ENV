...
function build_kernelconfig() {
	...
	local kernel_dir="${SDK_SYSDRV_DIR}/source/kernel"
	
	echo "===============kernel menuconfig====================="
	
	make distclean -C $kernel_dir
	make clean -C $kernel_dir
	make ARCH=arm CROSS_COMPILE=arm-rockchip831-linux-uclibcgnueabihf- rv1103b_defconfig O=$SDK_SYSDRV_DIR/source/objs_kernel -C $kernel_dir
	make ARCH=arm CROSS_COMPILE=arm-rockchip831-linux-uclibcgnueabihf- menuconfig O=$SDK_SYSDRV_DIR/source/objs_kernel -C $kernel_dir

	# copy config file to ${RK_KERNEL_DEFCONFIG}
	cp $SDK_SYSDRV_DIR/source/objs_kernel/.config $kernel_dir/arch/arm/configs/rv1103b_defconfig
}

function build_kernel_back(){
	check_config RK_KERNEL_DTS RK_KERNEL_DEFCONFIG || return 0
        local kernel_dir="${SDK_SYSDRV_DIR}/source/kernel"
	...
	echo "kernel_build_options==$kernel_build_options"
        cp $kernel_dir/arch/arm/configs/rv1103b_defconfig $SDK_SYSDRV_DIR/source/objs_kernel/.config
	make kernel -j9 -C $kernel_dir ARCH=arm CROSS_COMPILE=arm-rockchip831-linux-uclibcgnueabihf- O=$SDK_SYSDRV_DIR/source/objs_kernel
	
	if [ "$RK_ENABLE_FASTBOOT" = "y" ]; then
		mkdir -p $RK_PROJECT_PATH_FASTBOOT
		cp -ra $SDK_SYSDRV_DIR/source/objs_kernel/arch/arm/boot/Image $RK_PROJECT_PATH_FASTBOOT/
		cp -ra $SDK_SYSDRV_DIR/source/objs_kernel/arch/arm/boot/dts/rv1106b-evb2-v10-emmc.dtb  $RK_PROJECT_PATH_FASTBOOT/ 
	fi
...
function __RELEASE_FILESYSTEM_FILES()
{
	...
	if [ -d "$_target_dir" -a -n "${RK_PROJECT_TOOLCHAIN_CROSS}" ]; then
		...
		find "$_target_dir" -type f \( -perm /111 -o -name '*.so*' \) \
			-not \( -name 'libpthread*.so*' -o -name 'ld-*.so*' -o -name '*.ko' -o -name 'bt_service' \) -print0 | \
...
EOF
	...
	# 部署 OEM 资源 (from project/oem)
	# Binaries (including guard, ai-core, bluealsa, etc.)
	if [ -d "$PROJECT_TOP_DIR/oem/bin" ]; then
		mkdir -p $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin
		cp -rfa $PROJECT_TOP_DIR/oem/bin/* $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/
		chmod 777 $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/*
	fi

	# Resources (PCM)
	if [ -d "$PROJECT_TOP_DIR/oem/resources" ]; then
		mkdir -p $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/resources
		cp -rfa $PROJECT_TOP_DIR/oem/resources/* $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/resources/
	fi
}
...
EOF
   ...
   # WiFi/蓝牙芯片驱动部署
   # 使用环境变量 RK_WIFI_CHIP 控制，默认为 rk962
   # 可选值: rk962 (默认), rtl8723ds
   RK_WIFI_CHIP=${RK_WIFI_CHIP:-rk962}
   
   mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
   
   if [ "$RK_WIFI_CHIP" = "rtl8723ds" ]; then
       # RTL8723DS 驱动和固件部署
       echo "Deploying RTL8723DS WiFi/BT driver..."
       mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/lib/firmware/rtlbt
       cp $PROJECT_TOP_DIR/rtl8723d_config.dat-2M  $RK_PROJECT_PACKAGE_ROOTFS_DIR/lib/firmware/rtlbt/rtl8723d_config
       cp $PROJECT_TOP_DIR/rtl8723d_fw $RK_PROJECT_PACKAGE_ROOTFS_DIR/lib/firmware/rtlbt/
       cp $PROJECT_TOP_DIR/rtkhci $RK_PROJECT_PACKAGE_ROOTFS_DIR/
       cp $PROJECT_TOP_DIR/rtl8723ds.ko  $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       chmod 666 $RK_PROJECT_PACKAGE_ROOTFS_DIR/lib/firmware/rtlbt/rtl8723d_*
       chmod 777 $RK_PROJECT_PACKAGE_ROOTFS_DIR/rtkhci
       # 写入芯片类型标识
       echo "rtl8723ds" > $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/wifi_chip
   else
       # RK962 驱动和固件部署 (默认)
       echo "Deploying RK962 WiFi/BT driver..."
       # 复制驱动模块 (驱动文件应放在 project/drivers/ 目录下)
       cp $PROJECT_TOP_DIR/drivers/rk96x.ko $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       cp $PROJECT_TOP_DIR/drivers/mac80211.ko $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       cp $PROJECT_TOP_DIR/drivers/libarc4.ko $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       cp $PROJECT_TOP_DIR/drivers/ctr.ko $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       cp $PROJECT_TOP_DIR/drivers/ccm.ko $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       cp $PROJECT_TOP_DIR/drivers/aes_generic.ko $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ko/
       # 创建固件目录并复制固件
       mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw
       cp $PROJECT_TOP_DIR/drivers/rk96x_wifi_loader.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw/
       cp $PROJECT_TOP_DIR/drivers/rk962_wifi_v2.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw/
       cp $PROJECT_TOP_DIR/drivers/rk962_wifi_rf_v2.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw/
       cp $PROJECT_TOP_DIR/drivers/rk96x_bt_loader.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw/
       cp $PROJECT_TOP_DIR/drivers/rk962_bt_v2.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw/
       cp $PROJECT_TOP_DIR/drivers/rk962_bt_rf_v2.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/rk96x/fw/
       # 复制 RK962 专用启动脚本
       cp $RK_PROJECT_PATH_APP/rootfs_staging/etc/init.d/../../../app/wifi/start_wifi_rk962.sh $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/ 2>/dev/null || \
       cp $PROJECT_TOP_DIR/app/wifi/start_wifi_rk962.sh $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/
       chmod a+x $RK_PROJECT_PACKAGE_ROOTFS_DIR/../oem/usr/start_wifi_rk962.sh
       # 写入芯片类型标识
       echo "rk962" > $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/wifi_chip
   fi

   # 通用蓝牙工具部署 (两种芯片共用)
   cp $PROJECT_TOP_DIR/hciconfig $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/hcidump $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/hcitool $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/bluetoothctl $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/sdptool $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/display $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/dbus-send $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/dbus-monitor $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/wpa_supplicant $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/wpa_cli $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin
   cp $PROJECT_TOP_DIR/swapon $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/sbin/
   cp $PROJECT_TOP_DIR/swapoff $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/sbin/
   cp $PROJECT_TOP_DIR/mkswap $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/sbin/



   chmod 777 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/hci*
   chmod 777 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/sdptool
   chmod 777 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/blue*
   chmod 777 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/display
   chmod 777 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/dbus*

	    
   if [ ! -d "$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/dbus-1/system.d/bluetooth.conf" ];then
	mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/dbus-1/system.d/
	cp $PROJECT_TOP_DIR/bluetooth.conf $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/dbus-1/system.d/
   fi	    

   if [ ! -f "$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/wpa_supplicant.conf" ];then
	cp $PROJECT_TOP_DIR/wpa_supplicant.conf	$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/
   fi	    


   if [ ! -d "$RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/dbus-1/system.d/bluealsa.conf" ];then
	mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/dbus-1/system.d/
	cp $PROJECT_TOP_DIR/bluealsa.conf $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/dbus-1/system.d/
   fi
	...
	# Guard Service Script (from project/oem/init.d)
	if [ -f "$PROJECT_TOP_DIR/oem/init.d/S53_guard" ]; then
		cp -f $PROJECT_TOP_DIR/oem/init.d/S53_guard $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/
		sed -i 's/\r$//' $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S53_guard
		chmod a+x $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S53_guard
	fi

# Wi-Fi Integration
    if [ -d "$RK_PROJECT_PATH_APP/rootfs_staging/etc" ]; then
	# GPIO Setup Integration
	if [ -f "$RK_PROJECT_PATH_APP/rootfs_staging/etc/init.d/S50_gpio_setup" ]; then
		cp -f $RK_PROJECT_PATH_APP/rootfs_staging/etc/init.d/S50_gpio_setup $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/
		sed -i 's/\r$//' $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S50_gpio_setup
		chmod a+x $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S50_gpio_setup
	fi
	# Copy S51_wifi and create symlink
        if [ -f "$RK_PROJECT_PATH_APP/rootfs_staging/etc/init.d/S51_wifi" ]; then
             cp -f $RK_PROJECT_PATH_APP/rootfs_staging/etc/init.d/S51_wifi $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/
             sed -i 's/\r$//' $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S51_wifi
             chmod a+x $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S51_wifi

             # Create Symlink in rcS.d (for auto-start)
             mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/rcS.d
             ln -sf ../init.d/S51_wifi $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/rcS.d/S51_wifi
        fi

	# Copy S52_bluetooth and create symlink (from project/oem/init.d)
        if [ -f "$PROJECT_TOP_DIR/oem/init.d/S52_bluetooth" ]; then
             cp -f $PROJECT_TOP_DIR/oem/init.d/S52_bluetooth $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/
             sed -i 's/\r$//' $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S52_bluetooth
             chmod a+x $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/init.d/S52_bluetooth

             # Create Symlink in rcS.d (for auto-start)
             mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/rcS.d
             ln -sf ../init.d/S52_bluetooth $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/rcS.d/S52_bluetooth
        fi
	...
	# Bluetooth Libraries (from project/oem/lib)
        if [ -d "$PROJECT_TOP_DIR/oem/lib" ]; then
             mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
             cp -fa $PROJECT_TOP_DIR/oem/lib/* $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib/
        fi

	# Bluetooth Firmware (from project/oem/firmware)
        if [ -d "$PROJECT_TOP_DIR/oem/firmware" ]; then
             mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/../userdata/rx96x/fw
             cp -fa $PROJECT_TOP_DIR/oem/firmware/*.bin $RK_PROJECT_PACKAGE_ROOTFS_DIR/../userdata/rx96x/fw/
        fi
    fi

	# OEM share data (must be synced to target /usr/share)
	if [ -d "$PROJECT_TOP_DIR/oem/share" ]; then
		mkdir -p $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/share
		cp -faR $PROJECT_TOP_DIR/oem/share/. $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/share/
	fi
}
...
function __RUN_POST_CLEAN_FILES() {
	...
	if [ -n "$RK_NPU_MODEL" ];then
		for ii in `find $dir_find -name "iva_*.data"`
		do
			remove_item="$ii"
		...
		if [ "$RK_NPU_MODEL" = "NONE" ];then
			todo_files=`find $dir_find -name "iva_*.data" || echo ""`
			if [ -n "$todo_files" ];then
				rm -rfv $todo_files
			fi
		fi
...
do
	case $1 in
		...
		dm-v) shift; option="build_device_mapper_verity $@";break;;
		dm-crypt) shift; option="build_device_mapper_crypt $@";break;;
