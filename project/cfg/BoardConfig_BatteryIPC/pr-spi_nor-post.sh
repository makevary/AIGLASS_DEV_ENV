...
shopt -s extglob
rm !(ao_record_service|ao_record_demo|rk_mpi_uvc|usb_config.sh)
shopt -u extglob
cd -
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/librkaiq.so $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/librkrawstream.so $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/librga.so $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/libdrm.so $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/libdrm.so.2 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/libdrm.so.2.4.0 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/libfreetype.so.6 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/libfreetype.so.6.17.0 $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
mv $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/librkmuxer.so $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib
...
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/iqfiles/ps5458_PR01_24IRC-F20.bin
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/etc/iqfiles/sc230ai_PR01_24IRC-F20.bin
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/rkaiq_tool_server-V2.0.3-uclibc_1103b-NewToolChain
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/bin/gdb
...
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/sbin/v4l2-dbg
...
media-ctl -d /dev/media2 -l '"rkcif-mipi-lvds1":0->"rkisp-isp-subdev":0[0]'
media-ctl -d /dev/media2 -l '"rkisp_rawrd2_s":0->"rkisp-isp-subdev":0[1]'
media-ctl -d /dev/media2 --set-v4l2 '"rkisp-isp-subdev":0[fmt:SBGGR8_1X8/640x368]'
media-ctl -d /dev/media2 --set-v4l2 '"rkisp-isp-subdev":0[crop:(0,0)/640x368]'
media-ctl -d /dev/media2 --set-v4l2 '"rkisp-isp-subdev":2[fmt:YUYV8_2X8/640x368]'
media-ctl -d /dev/media2 --set-v4l2 '"rkisp-isp-subdev":2[crop:(0,0)/640x368]'
if [ -z "\$NoAuto" ]; then
	echo "start demo" > /dev/kmsg
	if [ -n "\$persist_camera_engine_log" ]; then
		/oem/usr/bin/ao_record_demo &>/tmp/rkaiq.log &
	else
		/oem/usr/bin/ao_record_demo &
	fi
fi
...
function run_delay_task(){
	...
	/usr/bin/tinyplay /oem/usr/share/enter_preroll_2ch.wav -p 2048 -D 0 -d 0

	export LD_LIBRARY_PATH=/mnt/sdcard:/oem/usr/lib:/oem/lib:
	mount /dev/mmcblk1p1 /mnt/sdcard/

	sleep 0.1
	cd /mnt/sdcard/
	test ! -f FakeCamera0.bin || ln /mnt/sdcard/FakeCamera0.bin /tmp/FakeCamera0.bin -s
	cd /tmp/
	test -f FakeCamera0.bin || ln /oem/usr/share/iqfiles/FakeCamera0.bin /tmp/FakeCamera0.bin -s
	sync
	...
	dmesg | grep -E 'thunderboot_time|csi2_dphy_s_stream stream on | as init process' >> /tmp/thunderboot_time.txt
	if [ -z "\$NoAuto" ]; then
		echo "start demo" > /dev/kmsg
		if [ -n "\$persist_camera_engine_log" ]; then
			/oem/usr/bin/ao_record_service &>/tmp/rkaiq1.log
		else
			/oem/usr/bin/ao_record_service
		fi
	fi
