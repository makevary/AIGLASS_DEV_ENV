...
rm librockauto.so
rm libivs.so
rm librkAVS_genLutAndStitch.so
rm librkrawstream.so
...
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/gc2053.ko
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/sc530ai.ko
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/os04a10.ko
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/8189fs.ko
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/ipv6.ko
...
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/sample_vi_vo
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_test.sh
...
# rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/rk_mpi_venc_test
...
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_change_resolution
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_chn_sharebuf
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_combo
...
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_rtsp_dev_chn_sharebuf
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_rtsp_eptz
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_rtsp_three_camera
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_svc_rtsp
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_bind_venc_wrap
...
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_get_frame_send_vo
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/simple_vi_get_frame_tde
rm -f $RK_PROJECT_PACKAGE_OEM_DIR/usr/bin/vpu_api_test

echo "================================================================================"
if find $RK_PROJECT_PACKAGE_ROOTFS_DIR $RK_PROJECT_PACKAGE_OEM_DIR -type f -name "rkaudio*.rknn" |grep -w $RK_AUDIO_MODEL;then
	echo "Found config RK_AUDIO_MODEL: $RK_AUDIO_MODEL"
	rm -fv `find $RK_PROJECT_PACKAGE_ROOTFS_DIR $RK_PROJECT_PACKAGE_OEM_DIR -name "rkaudio*.rknn" | grep -v $RK_AUDIO_MODEL`
fi
if [ "$RK_AUDIO_MODEL" = "NONE" ];then
	rm -fv `find $RK_PROJECT_PACKAGE_ROOTFS_DIR $RK_PROJECT_PACKAGE_OEM_DIR -name "rkaudio*.rknn" `
fi
echo "================================================================================"

echo "================================================================================"
if find $RK_PROJECT_PACKAGE_ROOTFS_DIR $RK_PROJECT_PACKAGE_OEM_DIR -type f -name "rkmodel_*.aiisp" |grep -w $RK_AIISP_MODEL;then
	echo "Found config RK_AIISP_MODEL: $RK_AIISP_MODEL"
	rm -fv `find $RK_PROJECT_PACKAGE_ROOTFS_DIR $RK_PROJECT_PACKAGE_OEM_DIR -name "rkmodel_*.aiisp" | grep -v $RK_AIISP_MODEL`
fi
if [ "$RK_AIISP_MODEL" = "NONE" ];then
	rm -fv `find $RK_PROJECT_PACKAGE_ROOTFS_DIR $RK_PROJECT_PACKAGE_OEM_DIR -name "rkmodel_*" `
fi
echo "================================================================================"

if [ "${RK_ENABLE_ROCKIT_MCU}x" = "yx" ];then
	line_num=$(sed -n -e '/rockit.ko/=' $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/insmod_ko.sh)
	sed -i "${line_num}c __insmod rockit.ko mcu_fw_path=\"./hpmcu_wrap.bin\" mcu_fw_addr=0x210f6000 isp_max_h=\${sensor_height} en_mcu=1" $RK_PROJECT_PACKAGE_OEM_DIR/usr/ko/insmod_ko.sh
fi