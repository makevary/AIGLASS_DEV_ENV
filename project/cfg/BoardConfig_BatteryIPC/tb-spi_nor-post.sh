...
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib/libv4l -rf
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/lib/libv4l*
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/v4l2-ctl
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/sbin/v4l2-dbg
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/usr/bin/media-ctl
...
if [ "$RK_APP_TYPE" = "RK_FASTBOOT_SINGLE_PROCESS" ];then
	if [ -z "\$NoAuto" ]; then
	...
	if [ -z "\$NoAuto" ]; then
		echo "start servr" > /dev/kmsg
		if [ -n "\$persist_camera_engine_log" ]; then
			/oem/usr/bin/fastboot_server >/tmp/rkaiq.log &
		else
			/oem/usr/bin/fastboot_server
		fi
	fi
	sh /etc/init.d/${PARTITION_MOUNT_SCRIPT} linkdev &
	if [ -z "\$NoAuto" ]; then
		echo "start demo" > /dev/kmsg
		if [ -n "\$persist_camera_engine_log" ]; then
			/oem/usr/bin/fastboot_client &>/tmp/rkaiq1.log &
		else
			/oem/usr/bin/fastboot_client &
		fi
	fi
...
for item in `echo rkipc fastboot_demo fastboot_client fastboot_server`
do
