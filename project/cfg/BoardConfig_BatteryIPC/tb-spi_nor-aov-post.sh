...
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_multi_vi
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_multi_vi_iva_venc
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_multi_vi_nonblend_stitch_venc
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_multi_vi_venc
...
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/object_detection_v3_cls8.data
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/object_detection_v3_pvn.data
rm $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/face_quality_v2.data
if [ "${RK_AOV_APP_BUILD_STATIC}x" = "yx" ];then
	...
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/lib/librkmuxer.so
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_vi
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_vi_venc
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_aiisp_iva_venc
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_npu_test
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_bind_test
	rm -vf $RK_PROJECT_PACKAGE_ROOTFS_DIR/oem/usr/bin/sample_aov_audio
fi
...
test -z "$RK_AOV_DEFAULT_APP" || \
	test -n "\$NoAuto" || \
	$RK_AOV_DEFAULT_APP --enable_save_mp4 1 --boot_frame 300 --wrap_lines 0 -r 1 -w \$rk_cam_w -h \$rk_cam_h \$tb_test_opt &
