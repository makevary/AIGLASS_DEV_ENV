...
#if defined(IS_NAND_FLASH) && defined(RUN_ON_BOARD)
...
#if defined(IS_NAND_FLASH) && defined(RUN_ON_BOARD)
extern int flash_write_buf(char *file_path, char *input_buf, size_t start, size_t len);
extern int flash_read_buf(char *file_path, char *output_buf, size_t start, size_t len);
#else
...
static uint32_t g_rk_cam1_max_fps = 30;
static uint32_t g_rk_cam2_max_fps = 30;
static uint32_t g_rk_cam_hdr = 0;
static uint32_t g_rk_cam2_hdr = 0;
static uint32_t g_rk_cam_index = 0;
static uint32_t g_cam_skip_frame_interval = 0;
static uint32_t g_rk_cam_num = 1;
static uint32_t g_rk_fastae_max_frame = 0;
static uint32_t g_rk_fastae_tolerance_coef = 0;
static uint32_t g_rk_rtt_stream_on_threshold = 0;
static uint32_t g_rk_rtt_stream_buf_cnt = 2;
...
#define uswap_32(x) \
	...
	char *rtt_log_param = NULL;

	if(!access("/proc/device-tree/reserved-memory/ramoops@rtos_log/reg", F_OK))
		rtt_log_param = "/proc/device-tree/reserved-memory/ramoops@rtos_log/reg";
	if(!access("/proc/device-tree/reserved-memory/ramoops@7c000/reg", F_OK))
		rtt_log_param = "/proc/device-tree/reserved-memory/ramoops@7c000/reg";
...
int dump_meta(const char *path)
{
	...
	if (app->head != *app_head_p) {
		...
		printf("  %s    : %d\n",RK_CAM1_MAX_FPS, app->cam1_max_fps);
		printf("  %s         : %d\n",RK_CAM2_FPS, app->cam2_fps);
		printf("  %s    : %d\n",RK_CAM2_MAX_FPS, app->cam2_max_fps);
		printf("  %s   : %d\n",RK_CAM_SKIP_FRAME_INTERVAL , app->cam_skip_frame_interval);
		...
		printf("  %s       : %d\n",RK_FASTAE_TOLERANCE_COEF, app->fastae_tolerance_coef);
		printf("  %s       : %d\n",RK_RTT_STREAM_ON_THRESHOLD, app->rtt_stream_on_threshold);
		printf("  %s       : %d\n",RK_RTT_STREAM_BUF_CNT, app->rtt_stream_buf_cnt);
	...
	if (app_bak->head != *(uint32_t *)g_app_param_head) {
		...
		printf("  %s    : %d\n",RK_CAM1_MAX_FPS, app_bak->cam1_max_fps);
		printf("  %s         : %d\n",RK_CAM2_FPS, app_bak->cam2_fps);
		printf("  %s    : %d\n",RK_CAM2_MAX_FPS, app_bak->cam2_max_fps);
		printf("  %s   : %d\n",RK_CAM_SKIP_FRAME_INTERVAL , app_bak->cam_skip_frame_interval);
		...
		printf("  %s       : %d\n",RK_FASTAE_TOLERANCE_COEF, app_bak->fastae_tolerance_coef);
		printf("  %s       : %d\n",RK_RTT_STREAM_ON_THRESHOLD, app_bak->rtt_stream_on_threshold);
		printf("  %s       : %d\n",RK_RTT_STREAM_BUF_CNT, app_bak->rtt_stream_buf_cnt);
...
static int usage(void)
{
	...
	printf("  --%s <size>               The max fps of camera sensor\n", RK_CAM1_MAX_FPS);
	printf("  --%s <size>               The max fps of secondary camera sensor\n", RK_CAM2_MAX_FPS);
	printf("  --%s <count>                  Set cam skip frame interval. \n", RK_CAM_SKIP_FRAME_INTERVAL);
...
/***camera sensor mirror flip end }}}**************************************/
	...
	printf("  --%s <count>          The tolerance coef of the sensor fast AE (Automatic Exposure) range: <0~100>. \n", RK_FASTAE_TOLERANCE_COEF);
	printf("  --%s <count>   RTT stream on when the sensor fast AE (Automatic Exposure) is matched within the count. \n", RK_RTT_STREAM_ON_THRESHOLD);
	printf("  --%s <count>        The stream buf alloc for RTT stream on (min 3). \n", RK_RTT_STREAM_BUF_CNT);
...
static int parse_args (int argc, char *argv[])
{
	...
	if (g_update) {
		...
		g_rk_cam1_max_fps = app_cfg->cam1_max_fps;
		g_rk_cam2_max_fps = app_cfg->cam2_max_fps;
		...
		g_cam_skip_frame_interval = app_cfg->cam_skip_frame_interval;

		g_rk_fastae_max_frame = app_cfg->fastae_max_frame;
		g_rk_fastae_tolerance_coef = app_cfg->fastae_tolerance_coef;
		g_rk_rtt_stream_on_threshold = app_cfg->rtt_stream_on_threshold;
		g_rk_rtt_stream_buf_cnt = app_cfg->rtt_stream_buf_cnt;
	...
	while (1) {
		...
		struct option long_options[] = {
			...
			{RK_CAM1_MAX_FPS, required_argument , 0 , 0} ,
			{RK_CAM2_MAX_FPS, required_argument , 0 , 0} ,
			{RK_CAM_SKIP_FRAME_INTERVAL , required_argument , 0 , 0} ,
			...
			{RK_RTT_STREAM_ON_THRESHOLD , required_argument , 0 , 0} ,
			{RK_RTT_STREAM_BUF_CNT , required_argument , 0 , 0} ,
		...
		case 0:
			...
			} else if (strcmp(option, RK_CAM1_MAX_FPS) == 0) {
				if (get_param_val(&g_rk_cam1_max_fps, optarg) == 0) {
					g_app_param_need = META_TRUE;
					g_cmdline_need = META_TRUE;
				}
			} else if (strcmp(option, RK_CAM2_MAX_FPS) == 0) {
				if (get_param_val(&g_rk_cam2_max_fps, optarg) == 0) {
					g_app_param_need = META_TRUE;
					g_cmdline_need = META_TRUE;
				}
			...
			} else if (strcmp(option, RK_CAM_SKIP_FRAME_INTERVAL) == 0) {
				if (get_param_val(&g_cam_skip_frame_interval, optarg) == 0) {
					g_cmdline_need = META_TRUE;
				}
			...
			} else if (strcmp(option, RK_FASTAE_TOLERANCE_COEF) == 0) {
				if (get_param_val(&g_rk_fastae_tolerance_coef, optarg) == 0) {
					g_app_param_need = META_TRUE;
				}
			} else if (strcmp(option, RK_RTT_STREAM_ON_THRESHOLD) == 0) {
				if (get_param_val(&g_rk_rtt_stream_on_threshold, optarg) == 0) {
					g_app_param_need = META_TRUE;
				}
			} else if (strcmp(option, RK_RTT_STREAM_BUF_CNT) == 0) {
				if (get_param_val(&g_rk_rtt_stream_buf_cnt, optarg) == 0) {
					g_app_param_need = META_TRUE;
				}
...
int main(int argc, char *argv[])
{
	...
	if (g_app_param_need == META_TRUE || g_create) {
		...
		pApp->cam1_max_fps = g_rk_cam1_max_fps;
		pApp->cam2_max_fps = g_rk_cam2_max_fps;
		pApp->cam_skip_frame_interval = g_cam_skip_frame_interval;
		pApp->fastae_max_frame = g_rk_fastae_max_frame;
		pApp->fastae_tolerance_coef = g_rk_fastae_tolerance_coef;
		pApp->rtt_stream_on_threshold = g_rk_rtt_stream_on_threshold;
		pApp->rtt_stream_buf_cnt = g_rk_rtt_stream_buf_cnt;
	...
	if (g_cmdline_need == META_TRUE) {
		...
		if (g_update) {
			...
			_strings_remove(cmd_default, KEY_EQUAL(RK_CAM1_MAX_FPS));
			_strings_remove(cmd_default, KEY_EQUAL(RK_CAM2_MAX_FPS));
			_strings_remove(cmd_default, KEY_EQUAL(RK_CAM_SKIP_FRAME_INTERVAL));
