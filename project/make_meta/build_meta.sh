...
function msg_help()
{
	...
	echo "        --main_sensor_iq     input main camera sensor's iq bin file."
	echo "        --second_sensor_iq   input secondary camera sensor's iq bin file."
	echo "        --cam_iqfile         input camera sensor IQ file."
	echo "        --media_dir          the dir of meta header files install"
	echo "        --hardware           the hardware type"
	exit 0
}
...
function chk_param()
{
	BUILD_META_PARAMS="-v --version -h --help -c --clean --main_sensor_iq --second_sensor_iq -e --export --cam_iqfile --meta_param --output --rootfs_dir --media_dir --pc_tools_dir --boot_medium --tiny_meta --hardware"
	...
	if [ -z "$rk_hardware" ];then
		msg_error "invalid parameter, please set evb hardware info"
		exit 1
	fi
}
...
make -C ${cwd}/sensor_init cam1_w=$cam1_w cam1_h=$cam1_h cam2_w=$cam2_w cam2_h=$cam2_h  hardware_info=$rk_hardware
