...
RK_S32 draw_rect_2bpp_1bpp(RK_U8 *buffer, RK_U32 width, RK_U32 height, int rgn_x,
                           int rgn_y, int rgn_w, int rgn_h, int line_pixel,
                           COLOR_INDEX_E color_index, int format) {
	int i;
	RK_U8 *ptr = buffer;
	RK_U8 value = 0;
	int shiftValue = 0;
	if (format == RK_FMT_2BPP) // 2bpp
		shiftValue = 2;
	else if (format == RK_FMT_1BPP)
		shiftValue = 3; // 1bpp
	...
	ptr += (width * rgn_y + rgn_x) >> shiftValue;
	for (i = 0; i < line_pixel; i++) {
		memset(ptr, value, (rgn_w + 3) >> shiftValue);
		ptr += width >> shiftValue;
	}
	// draw letft/right line
	for (i = 0; i < (rgn_h - line_pixel * 2); i++) {
		if (color_index == RGN_COLOR_LUT_INDEX_1) {
			*ptr = rgn_color_lut_1_left_value[line_pixel - 1];
			*(ptr + ((rgn_w + 3) >> shiftValue)) =
			    rgn_color_lut_1_right_value[line_pixel - 1];
		} else {
			*ptr = rgn_color_lut_0_left_value[line_pixel - 1];
			*(ptr + ((rgn_w + 3) >> shiftValue)) =
			    rgn_color_lut_0_right_value[line_pixel - 1];
		}
		ptr += width >> shiftValue;
	}
	// draw bottom line
