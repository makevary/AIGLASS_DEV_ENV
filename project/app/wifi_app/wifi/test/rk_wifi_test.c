...
int rk_wifi_set_keepalive(int argc, char *argv[])
{
	...
	memset(kp.domain, 0, sizeof(kp.domain));
	memset(kp.port, 0, sizeof(kp.port));
	memset(kp.period, 0, sizeof(kp.period));
	memset(kp.device_id, 0, sizeof(kp.device_id));
	memset(kp.key, 0, sizeof(kp.key));
	...
	//temperature
	strncpy(kp.device_id, "87654321", 8);
	strncpy(kp.key, "12345678", 8);
