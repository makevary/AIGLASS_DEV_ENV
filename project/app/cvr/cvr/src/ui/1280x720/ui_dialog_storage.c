...
void ui_nosdcard_dialog_destroy(void) {
    ui_dialog_destroy();
}

void ui_noformat_dialog_destroy(void) {
    ui_dialog_destroy();
}

void ui_scansdcard_dialog_destroy(void) {
    ui_dialog_destroy();
}

void ui_nosdcard_dialog_create(void) {
    ui_dialog_create(RES_STR_SETTINGS_STORE,
                     RES_STR_SETTINGS_BOX_NOSDCARD);
}

void ui_noformat_dialog_create(void) {
    ui_dialog_create(RES_STR_SETTINGS_STORE,
                     RES_STR_SETTINGS_BOX_NOFORMAT);
}

void ui_scansdcard_dialog_create(void) {
    ui_dialog_create(RES_STR_SETTINGS_STORE,
                     RES_STR_SETTINGS_BOX_SCAN);
}

void ui_storage_status_dialog_destroy(void) {
    if (storage_status_dialog_obj) {
        ui_common_remove_stype_all(storage_status_dialog_obj);
...
void ui_storage_status_dialog_create(void) {
    ...
    lv_obj_add_event_cb(obj, storage_status_dialog_exit_cb, LV_EVENT_CLICKED, storage_status_dialog_obj);
    ...
    lv_label_set_text(obj, RES_STR_SETTINGS_BOX_STORE_TOTAL);
    ...
    mpu_storage_get_capacity(&sto_total, &sto_free);
    ...
    if (progress > 100)
        progress = 100;
    sprintf(info, "%s %dGB/%s %dGB", RES_STR_SETTINGS_BOX_STORE_INFO, sto_use, RES_STR_SETTINGS_BOX_STORE_TOTAL, sto_total);
