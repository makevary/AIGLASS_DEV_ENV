...
static LANG_PARSE_STATE_E lang_parse_line(const char *input_line, char *section,
                                          char *key, char *value);
...
static LANG_RES_LIST_S g_lang_array[LANG_TYPE_MAX] = {
    {
        .type =  LANG_TYPE_CHINESE,
        .file_path = CVR_TEXT_CHINESE_PATH,
        .res = NULL
    },
    {
        .type =  LANG_TYPE_ENGLISH,
        .file_path = CVR_TEXT_ENGLISH_PATH,
        .res = NULL
    }
};
/**********************
...
static int32_t lang_res_grow(LANG_RES_S *res) {
    ...
    if (NULL == vector) {
        CVR_ERR("Memory expansion failed.\n");
        return -1;
    }
...
static LANG_RES_S* lang_resource_file_load(const char *res_name) {
    ...
    while (NULL != fgets(line + last, LANG_LINE_MAX_SIZE - last, in)) {
        ...
        if (line[len] != '\n' && !feof(in)) {
            CVR_ERR("Input line too long in %s (%d)\n", res_name, lineno);
