...
typedef struct {
    ...
    volatile RKADK_PLAYER_EVENT_E event;
...
static void player_event_handler(RKADK_PLAYER_EVENT_E enEvent) {
    switch (enEvent) {
        ...
        case RKADK_PLAYER_EVENT_EOF:
            ...
            mpu_player_pause();
            if (NULL != icon_play_index)
                    lv_img_set_src(player_ctx.play_obj, icon_play_index);
            break;
        ...
        case RKADK_PLAYER_EVENT_ERROR:
            player_ctx.state = UI_PLAYER_STATE_ERR;
            CVR_INFO("+++++ RKADK_PLAYER_EVENT_ERROR +++++\n");
            break;
        case RKADK_PLAYER_EVENT_PREPARED:
            player_ctx.state = UI_PLAYER_STATE_PREPARED;
        ...
        case RKADK_PLAYER_EVENT_PLAY:
            ...
            if (NULL != icon_pause_index)
                lv_img_set_src(player_ctx.play_obj, icon_pause_index);
            break;
...
static RKADK_VOID player_event_cb(RKADK_MW_PTR Player,
                                  ...
                                  RKADK_VOID *pData) {
    player_ctx.event = enEvent;
}

static void time_handler(lv_timer_t *timer) {
    if (RKADK_PLAYER_EVENT_BUTT != player_ctx.event) {
        player_event_handler(player_ctx.event);
        player_ctx.event = RKADK_PLAYER_EVENT_BUTT;
    }
...
static void video_player_start(void) {
    ...
    ret = mpu_player_switch(player_ctx.file_info.path);
    if (ret) {
...
static int32_t video_player_create(void) {
    RKADK_RECT_S disp_area;

    memset(&disp_area, 0, sizeof(disp_area));
    disp_area.u32X = 0, disp_area.u32Y = 0;
    disp_area.u32Width = lv_disp_get_ver_res(NULL);
    disp_area.u32Height = lv_disp_get_hor_res(NULL);

    if (mpu_player_create(MPU_PLAYER_TYPE_VIDEO | MPU_PLAYER_TYPE_AUDIO, &disp_area,
...
static void video_player_create_ctrl(lv_obj_t *page_obj) {
    ...
    lv_obj_set_style_bg_color(page_obj, transp_color, 0);
    ...
    player_ctx.play_obj = obj = lv_img_create(page_obj);
    if (NULL != icon_play_index)
        lv_img_set_src(obj, icon_play_index);
    lv_obj_add_style(obj, &style, 0);
    ...
    lv_obj_set_size(obj, lv_pct(60) , 10);
...
static UI_PAGE_HANDLER_T video_player_page = {
    .name = "video_player",
    .init = NULL,
