...
static int drm_set_plane_zpos(void) {
    ...
    uint32_t prop_id = get_plane_property_id("ZPOS");

    if (!prop_id) {
        err("Couldn't find plane prop ZPOS");
        return -1;
    }

    for (i = 0; i < drm_dev.count_plane_props; ++i)
        if (!strcmp(drm_dev.plane_props[i]->name, "ZPOS"))
            break;
    ...
    ret = drmModeObjectSetProperty(drm_dev.fd, drm_dev.plane_id, DRM_MODE_OBJECT_PLANE, prop_id, zpos);
    if (ret < 0) {
...
#else
    drmModeSetPlane(drm_dev.fd, drm_dev.plane_id, drm_dev.crtc_id, buf->fb_handle, 0,
                    0, 0, drm_dev.width, drm_dev.height,
                    0, 0, drm_dev.width << 16, drm_dev.height << 16);
#endif
...
void drm_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    if (true == partial_update) {
        ...
        dst.fd = fbuf->buf_fd;
        dst.mmuFlag = 1;
        rga_set_rect(&src.rect, 0, 0, drm_dev.width, drm_dev.height, wstride, drm_dev.height, format);
        rga_set_rect(&dst.rect, 0, 0, drm_dev.width, drm_dev.height, wstride, drm_dev.height, format);
    ...
    if (drm_dev.rot == LV_DISP_ROT_90 || drm_dev.rot == LV_DISP_ROT_270) {
        if (w < 2 || h < 2) {
            ...
            rga_set_rect(&dst.rect, (drm_dev.width - (area->y1 + h)), area->x1, h, w, wstride, drm_dev.height, format);
            if (c_RkRgaBlit(&src, &dst, NULL))
                printf("c_RkRgaBlit2 error : %s\n", strerror(errno));
        }
    } else {
