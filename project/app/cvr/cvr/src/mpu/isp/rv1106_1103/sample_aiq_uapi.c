...
static int SAMPLE_ISP_Init(RKADK_U32 u32CamId, SAMPLE_ISP_PARAM stIspParam) {
  ...
  if (NULL != stIspParam.iqFileDir) {
  ...
  } else if ((NULL != stIspParam.iqDataVirAddr) && (0 < stIspParam.iqDataVirAddr)) {
    ret = rk_aiq_uapi2_sysctl_preInit_scene(atAiqStaticInfo.sensor_info.sensor_name, "normal",
                                            stIspParam.color_mode ? "night" : "day");
    if (ret) {
      RKADK_LOGE("rk_aiq_uapi2_sysctl_preInit_scene u32CamId[%d] failed[%d]", u32CamId, ret);
      return ret;
    }

    ret = rk_aiq_uapi2_sysctl_preInit_iq_addr(atAiqStaticInfo.sensor_info.sensor_name,
                                              (void *)stIspParam.iqDataVirAddr, stIspParam.iqDataSize);
    if (ret) {
      RKADK_LOGE("rk_aiq_uapi2_sysctl_preInit_iq_addr u32CamId[%d] failed[%d]", u32CamId, ret);
      return ret;
    }
    ...
    ret = rk_aiq_uapi2_sysctl_preInit_tb_info(atAiqStaticInfo.sensor_info.sensor_name, &tb_info);
    if (ret) {
      RKADK_LOGE("rk_aiq_uapi2_sysctl_preInit_tb_info u32CamId[%d] failed[%d]", u32CamId, ret);
    }

    RKADK_LOGD("CamId: %d, sensor_name is %s, color_mode is %d", u32CamId,
              atAiqStaticInfo.sensor_info.sensor_name, stIspParam.color_mode);
  }
...
int SAMPLE_ISP_Start_Ex(RKADK_U32 u32CamId, SAMPLE_ISP_PARAM stIspParam) {
  int index, ret;

  RKADK_CHECK_CAMERAID(u32CamId, RKADK_FAILURE);
  RKADK_CHECK_POINTER(stIspParam.iqDataVirAddr, RKADK_FAILURE);

  if (0 >= stIspParam.iqDataSize) {
    RKADK_LOGE("invalid iqDataSize : %u", stIspParam.iqDataSize);
    return -1;
  }
