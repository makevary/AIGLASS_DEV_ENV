...
int sample_common_isp_init(int cam_id, rk_aiq_working_mode_t WDRMode, bool MultiCam,
	...
	LOG_INFO("rk_aiq_uapi2_sysctl_init over\n");
	if (!aiq_ctx) {
...
int sample_common_isp_run(int cam_id) {
	...
	LOG_INFO("rk_aiq_uapi2_sysctl_prepare succeed\n");
...
int rk_isp_set_white_blance_ct(int cam_id, int value) {
  ...
  ret = rk_aiq_uapi2_setMWBCT(rkipc_aiq_get_ctx(cam_id), value);
...
int rk_isp_deinit(int cam_id) {
	...
	RK_ISP_CHECK_CAMERA_ID(cam_id);
	LOG_INFO("%s", "rk_aiq_uapi2_sysctl_stop enter\n");
	rk_aiq_uapi2_sysctl_stop(g_aiq_ctx[cam_id], false);
