...
typedef struct _rkMpiVICtx {
	...
	RK_BOOL bIfOpenEptz;
	RK_BOOL bIfQuickStart;
	RK_BOOL bEnableShareBuf;
...
typedef struct _rkMpiIvaCtx {
	...
	ROCKIVA_BA_ResultCallback baResultCallback;
	ROCKIVA_DetectResultCallback detectResultCallback;
	ROCKIVA_FrameReleaseCallback releaseCallback;
	enum ROCKIVA_MODE eIvaMode;
	pthread_mutex_t stIvaResultMutex;
	struct LinkedList stIvaResultList;
} SAMPLE_IVA_CTX_S;
#endif
...
RK_S32 SAMPLE_COMM_AI_FdGetFrame(SAMPLE_AI_CTX_S *ctx, void **pdata);
...
int SAMPLE_COMM_MuxerClean(SAMPLE_MPI_MUXER_S *pstMuxer);
int get_int32_from_env(const char *key, int default_value);
