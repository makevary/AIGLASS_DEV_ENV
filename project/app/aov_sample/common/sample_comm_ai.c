...
RK_S32 SAMPLE_COMM_AI_FdGetFrame(SAMPLE_AI_CTX_S *ctx, void **pdata) {
	...
	ctx->s32DevFd = RK_MPI_AI_GetFd(ctx->s32DevId, ctx->s32ChnId);
	FD_ZERO(&readfds);
	FD_SET(ctx->s32DevFd, &readfds);
	timeout.tv_sec = 1;
	timeout.tv_usec = 0;
	s32Ret = select(ctx->s32DevFd + 1, &readfds, NULL, NULL, &timeout);
	if (s32Ret == -1) {
		perror("select");
		exit(EXIT_FAILURE);
	} else if (s32Ret) {
		if (FD_ISSET(ctx->s32DevFd, &readfds)) {
			s32Ret = RK_MPI_AI_GetFrame(ctx->s32DevId, ctx->s32ChnId, &ctx->stFrame,
			                            RK_NULL, s32MilliSec);
			if (s32Ret == 0) {
				*pdata = RK_MPI_MB_Handle2VirAddr(ctx->stFrame.pMbBlk);
			} else {
