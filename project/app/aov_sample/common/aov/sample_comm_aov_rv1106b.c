...
#define SUSPEND_TIME_REG 0x20514094
...
#define SDCARD_DEVICE "20d20000.mmc"
#define SDCARD_DRIVER_PREPARED "bind@/devices/platform/20d20000.mmc"
#define SDCARD_BIND_DONE "bind@/devices/platform/20d20000.mmc/mmc_host/mmc1/mmc1"
#define SDCARD_UNBIND_DONE "unbind@/devices/platform/20d20000.mmc"
#define SDCARD_DRIVER "/sys/bus/platform/drivers/dwmmc_rockchip"

static bool detect_sdcard_is_enable() {
	int value = 0;
	if (SAMPLE_COMM_ReadReg(0x20D20050, (RK_U8 *)&value, sizeof(value)) != RK_SUCCESS)
		return RK_FALSE;
	if (value == 0)
		return RK_TRUE;
	else
...
#define DWC3_DEVICE "20b00000.usb"
#define DWC3_DRIVER "/sys/bus/platform/drivers/dwc3/"
#define USB_DEVICE "xhci-hcd.0.auto"
#define USB_BIND_DONE "bind@/devices/platform/usbdrd/20b00000.usb"
#define USB_UNBIND_DONE "unbind@/devices/platform/usbdrd/20b00000.usb"
...
int SAMPLE_COMM_AOV_BindUsb() {
	...
	struct timeval timeout;
	struct sockaddr_nl addr;
	if (device_driver_is_bound(DWC3_DEVICE, DWC3_DRIVER)) {
	...
	timeout.tv_usec = MAX_SELECT_TIMEOUT;
	// bind dwc3
	ret = device_attach_driver(DWC3_DEVICE, DWC3_DRIVER);
	if (ret != RK_SUCCESS)
		goto __FAILED;
	// wait for bind success
__RETRY:
...
int SAMPLE_COMM_AOV_UnbindUsb() {
	...
	timeout.tv_usec = MAX_SELECT_TIMEOUT;
	// unbind dwc3
	ret = device_detach_driver(DWC3_DEVICE, DWC3_DRIVER);
	if (ret != RK_SUCCESS)
		goto __FAILED;
	// wait for unbind success
__RETRY:
...
#define SDIO_DEVICE "20650000.mmc"
#define SDIO_BIND_DONE "bind@/devices/platform/20650000.mmc"
#define SDIO_UNBIND_DONE "unbind@/devices/platform/20650000.mmc"
#define SDIO_DRIVER "/sys/bus/platform/drivers/dwmmc_rockchip"

int SAMPLE_COMM_AOV_BindSDIO() {
	int ret = 0;
	int fd = -1;
	char buf[MAX_NL_BUF_SIZE] = {'\0'};
	fd_set read_set;
	...
	if (device_driver_is_bound(SDIO_DEVICE, SDIO_DRIVER)) {
		printf("[%s()] sdcard device already bind!\n", __func__);
		return RK_SUCCESS;
	}
	...
	// bind sdcard
	if (device_attach_driver(SDIO_DEVICE, SDIO_DRIVER) != RK_SUCCESS)
		goto __FAILED;
	// wait for bind success
__RETRY:
	ret = select(fd + 1, &read_set, NULL, NULL, &timeout);
	if (ret > 0) {
		...
		buf[MAX_NL_BUF_SIZE - 1] = '\0';
		// printf("[%s()] bind msg: %s\n", __func__, buf);
		if (strncmp(buf, SDIO_BIND_DONE, strlen(SDIO_BIND_DONE)) == 0) {
			printf("[%s()] Bind success: %s\n", __func__, buf);
			goto __SUCCESS;
		}
		goto __RETRY; // drop all message
...
int SAMPLE_COMM_AOV_UnbindSDIO() {
	int ret = 0;
	int fd = -1;
	char buf[MAX_NL_BUF_SIZE] = {'\0'};
	fd_set read_set;
	...
	// unbind sdcard
	if (device_detach_driver(SDIO_DEVICE, SDIO_DRIVER) != RK_SUCCESS)
		goto __FAILED;
	// wait for unbind success
__RETRY:
	ret = select(fd + 1, &read_set, NULL, NULL, &timeout);
	if (ret > 0) {
		...
		buf[MAX_NL_BUF_SIZE - 1] = '\0';
		// printf("[%s()] unbind msg: %s\n", __func__, buf);
		if (strcmp(buf, SDIO_UNBIND_DONE) == 0) {
			printf("[%s()] Unbind success: %s\n", __func__, buf);
			goto __SUCCESS;
		}
		goto __RETRY; // drop all message
