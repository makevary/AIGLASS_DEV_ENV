...
RK_S32 SAMPLE_COMM_VI_CreateChn(SAMPLE_VI_CTX_S *ctx) {
	...
	if (s32Ret == RK_ERR_VI_NOT_CONFIG) {
		// 0-1.config dev
#if defined(RV1106B)
		ctx->stDevAttr.u32BufCount = ctx->u32CifBuffCnt;
#endif
	...
	// INFO: RV1106B must set share buffer for dual-cam wrap, this restriction does not
	// exist in RV1106.
	if (ctx->bEnableShareBuf) {
		if (ctx->s32DevId == 0) {
			ctx->stChnAttr.stIspOpt.stMaxSize.u32Height *= 2;
			ctx->stChnAttr.enAllocBufType = VI_ALLOC_BUF_TYPE_INTERNAL;
		} else {
			ctx->stChnAttr.enAllocBufType = VI_ALLOC_BUF_TYPE_CHN_SHARE;
		}
	} else if (ctx->s32DevId > 0 && ctx->bWrapIfEnable) {
#if defined(RV1106B)
		RK_LOGE("RV1106B must set share buffer for dual-cam wrap!");
		goto __FAILED1;
#endif
	}
