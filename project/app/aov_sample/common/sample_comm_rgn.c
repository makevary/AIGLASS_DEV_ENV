...
RK_S32 SAMPLE_COMM_RGN_CreateChn(SAMPLE_RGN_CTX_S *ctx) {
	...
	if (ctx->st_osd_data.enable) {
		...
		ctx->st_osd_data.text.font_color = 0x000000;
...
RK_S32 SAMPLE_COMM_RGN_DrawRectFromIVA(SAMPLE_RGN_CTX_S *ctx, SAMPLE_IVA_CTX_S *iva_ctx,
                                       RK_BOOL bDraw) {
...
#pragma GCC diagnostic ignored "-Wint-conversion"
	...
	struct timespec now;
	iva_result = SAMPLE_COMM_IVA_Pop_Result(iva_ctx);
	if (iva_result == NULL) {
		RK_LOGD("empty iva result list...\n");
	}
	s32Ret = RK_MPI_RGN_GetCanvasInfo(RgnHandle, &stCanvasInfo);
	if (s32Ret != RK_SUCCESS) {
		SAMPLE_COMM_IVA_Release_Result(iva_ctx, iva_result);
		RK_LOGE("RK_MPI_RGN_GetCanvasInfo failed with %#x!", s32Ret);
		return s32Ret;
	}

	if (ctx->u32BmpFormat == RK_FMT_1BPP)
		memset((void *)stCanvasInfo.u64VirAddr, 0,
		       stCanvasInfo.u32VirWidth * stCanvasInfo.u32VirHeight >> 3);
	else if (ctx->u32BmpFormat == RK_FMT_2BPP)
		memset((void *)stCanvasInfo.u64VirAddr, 0,
			...
			if (x >= 0 && y >= 0 && w > 0 && h > 0) {
				if (ctx->u32BmpFormat == RK_FMT_1BPP) {
					if (objectInfo->type == ROCKIVA_OBJECT_TYPE_PERSON) {
						draw_rect_2bpp_1bpp(
						    (RK_U8 *)stCanvasInfo.u64VirAddr, stCanvasInfo.u32VirWidth,
						    stCanvasInfo.u32VirHeight, x, y, w, h, line_pixel,
						    RGN_COLOR_LUT_INDEX_0, RK_FMT_1BPP);
					...
					if (objectInfo->type == ROCKIVA_OBJECT_TYPE_PERSON) {
						draw_rect_2bpp_1bpp(
						    (RK_U8 *)stCanvasInfo.u64VirAddr, stCanvasInfo.u32VirWidth,
						    stCanvasInfo.u32VirHeight, x, y, w, h, line_pixel,
						    RGN_COLOR_LUT_INDEX_0, RK_FMT_2BPP);
					} else if (objectInfo->type == ROCKIVA_OBJECT_TYPE_FACE) {
						draw_rect_2bpp_1bpp(
						    (RK_U8 *)stCanvasInfo.u64VirAddr, stCanvasInfo.u32VirWidth,
						    stCanvasInfo.u32VirHeight, x, y, w, h, line_pixel,
						...
						draw_rect_2bpp_1bpp(
						    (RK_U8 *)stCanvasInfo.u64VirAddr, stCanvasInfo.u32VirWidth,
						    stCanvasInfo.u32VirHeight, x, y, w, h, line_pixel,
						    RGN_COLOR_LUT_INDEX_1, RK_FMT_2BPP);
...
RK_S32 SAMPLE_COMM_RGN_DrawOsd(SAMPLE_RGN_CTX_S *ctx, char *aov_mode, RK_FLOAT Fps) {
	RK_S32 s32Ret = RK_SUCCESS;
	RK_U64 timestamp = 0LL;
	...
	timestamp = cur_time.tv_sec * 1000LL + cur_time.tv_nsec / 1000000LL;
	if (timestamp > ctx->u64LastUpdateTs &&
	    ((timestamp - ctx->u64LastUpdateTs) >= 500LL)) {
		ctx->u64LastUpdateTs = timestamp;
	} else {
		RK_LOGD("would not update osd! last update ts %lld, cur ts %lld",
		        ctx->u64LastUpdateTs, timestamp);
		return s32Ret;
	}
	// generate time string.
	RK_LOGD("rgn handle %d, mode %s, Fps %.1f\n", ctx->rgnHandle, aov_mode, Fps);

	sprintf(osd_append_char, "_%s_%.1fFPS", aov_mode, Fps);
...
RK_S32 SAMPLE_COMM_RGN_Draw2AInfo(SAMPLE_RGN_CTX_S *ctx, int cam_id) {
	RK_S32 s32Ret = RK_SUCCESS;
	RK_U64 timestamp = 0LL;
	struct timespec cur_time;
	BITMAP_S stBitmap;
	int wchar_cnt;
	char buf[MAX_WCH_BYTE] = {'\0'};
	...
	SAMPLE_COMM_ISP_Get2AInfoStr(buf, sizeof(buf), cam_id);
	wchar_cnt = swprintf(ctx->st_osd_data.text.wch, MAX_WCH_BYTE, L"%s", buf);
	RK_LOGD("wchar_cnt = %d\n", wchar_cnt);
	if (wchar_cnt <= 0) {
		RK_LOGE("swprintf error\n");
		return -1;
	}
	// calculate really buffer size and allocate buffer for time string.
	ctx->st_osd_data.width =
