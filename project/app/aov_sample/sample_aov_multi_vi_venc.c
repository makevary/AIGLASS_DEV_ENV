...
static RK_S32 venc_get_frame_and_save2sdcard(SAMPLE_VENC_CTX_S *ctx, char *const buffer,
	...
	if (g_cmd_args->bEnableSaveToSdcard) {
		if (buffer == NULL || *psize > g_cmd_args->u32CacheBuffSize) {
		...
		if ((*psize + frame.pstPack->u32Len) <= g_cmd_args->u32CacheBuffSize) {
	...
	if (g_cmd_args->bEnableRtsp) {
		...
		rtsp_tx_video(g_mpi_ctx->rtsp.sessions[ctx->s32ChnId], data,
		              frame.pstPack->u32Len, frame.pstPack->u64PTS);
		rtsp_do_event(g_mpi_ctx->rtsp.handle);
		pthread_mutex_unlock(&g_mpi_ctx->rtsp.mutex);
	}
	RK_LOGI("chn:%d, frame %d, len:%u, pts:%llu, seq:%u", ctx->s32ChnId, frame_num,
...
static void *venc_get_stream(void *pArgs) {
	...
	if (g_cmd_args->bEnableSaveToSdcard) {
		...
		main_buffer = (char *)malloc(g_cmd_args->u32CacheBuffSize); // 10M form 200 frame
		if (main_buffer == NULL) {
			RK_LOGE("malloc failed!");
			program_handle_error(__func__, __LINE__);
			return NULL;
		...
		sub_buffer = (char *)malloc(g_cmd_args->u32CacheBuffSize); // 10M form 200 frame
		if (sub_buffer == NULL) {
			RK_LOGE("malloc failed!");
			program_handle_error(__func__, __LINE__);
			return NULL;
