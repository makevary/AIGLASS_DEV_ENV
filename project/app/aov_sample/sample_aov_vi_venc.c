...
static RK_S32 g_enable_smart_ir = 0;
...
static RK_CHAR optstr[] = "?::a::b:w:h:l:o:e:d:D:I:i:L:M:r:f:";
static const struct option long_options[] = {
    ...
    {"max_exp_time", required_argument, NULL, 'm' + 'e' + 't'},
...
static void print_usage(const RK_CHAR *name) {
	...
	printf("\t-f | --fps : framerate of isp and venc, Default: 25\n");
	...
	printf("\t--max_exp_time: force AE max exp time, unit ms\n");
	printf("\t--enable_smart_ir : enable auto switch ircut and irled when day/night "
	       "switch, default: 0\n");
}

static void unbindAllDevice() {
...
static void *venc_get_stream(void *pArgs) {
	...
	for (int i = 0; i < g_u32BootFrame; i++) {
		...
		if (s32Ret == RK_SUCCESS) {
			...
			RK_LOGI("boot frame, chn:%d, loopCount:%d, len:%u, pts:%llu, seq:%u",
	...
	while (!quit) {
		...
		if (s32Ret == RK_SUCCESS) {
			...
			if (getenv("sample_aov_debug_drop_frame")) {
				RK_LOGE("venc drop frame, please check it\n");
			} else {
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
		...
		case 'q' + 's':
			...
			break;
		case 'm' + 'e' + 't':
			u32AEMaxExpTime = atoi(optarg);
			break;
		case 'e' + 's' + 'r':
			g_enable_smart_ir = (atoi(optarg) != 0) ? RK_TRUE : RK_FALSE;
			break;
		case '?':
		default:
			print_usage(argv[0]);
...
#endif
	if (u32AEMaxExpTime)
		SAMPLE_COMM_ISP_SetMaxExpTime(s32CamId, u32AEMaxExpTime);
	if (g_enable_smart_ir)
	...
	ctx->vi.stChnAttr.stIspOpt.u32BufCount = 1;
#endif
	...
	ctx->venc.u32Fps = u32Fps;
	ctx->venc.u32Gop = u32Fps * 2;
	...
	ctx->venc.u32BuffSize = video_width * video_height / 3;
	if (bWrapIfEnable) {
...
#endif
	SAMPLE_COMM_AOV_SetPeriodTime(s32SuspendTime);
