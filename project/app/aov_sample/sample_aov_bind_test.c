...
static RK_CHAR optstr[] = "?:l:s:e:a:o:u:i:";
...
static const struct option long_options[] = {
    ...
    {"enable_usb", optional_argument, NULL, 'u'},
    {"enable_sdio", optional_argument, NULL, 'i'},
...
static void print_usage(const RK_CHAR *name) {
	...
	printf("\t-u | --enable_usb: enable usb, Default 0\n");
	printf("\t-i | --enable_sdio: enable sdio, Default 0\n");
...
int main(int argc, char *argv[]) {
	...
	while ((c = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
		...
		case 'a':
			...
			break;
		case 'u':
			enable_usb = atoi(optarg);
			break;
		case 'i':
			enable_sdio = atoi(optarg);
			break;
		case 'o':
			output_path = optarg;
			break;
	...
	if (enable_audio) {
		...
		system("insmod /oem/usr/ko/snd-soc-rockchip.ko");
		system("insmod /oem/usr/ko/snd-soc-rockchip-sai.ko");
	...
	if (enable_usb) {
#if defined(RV1106B)
		system("insmod /oem/usr/ko/usb-common.ko");
		system("insmod /oem/usr/ko/usbcore.ko");
		system("insmod /oem/usr/ko/udc-core.ko");
		system("insmod /oem/usr/ko/xhci-hcd.ko");
		system("insmod /oem/usr/ko/xhci-plat-hcd.ko");
		system("insmod /oem/usr/ko/phy-rockchip-inno-usb2.ko");
		system("insmod /oem/usr/ko/dwc3-of-simple.ko");
		system("insmod /oem/usr/ko/dwc3.ko");
#endif
	...
	for (int i = 0; i < loop_count; ++i) {
		...
		if (enable_usb) {
			ret = SAMPLE_COMM_AOV_UnbindUsb();
			if (ret != RK_SUCCESS) {
				printf("[%s()] unbind usb failed!\n", __func__);
				break;
			}
		}
		if (enable_sdio) {
			ret = SAMPLE_COMM_AOV_UnbindSDIO();
			if (ret != RK_SUCCESS) {
				printf("[%s()] unbind sdoi failed!\n", __func__);
				break;
			}
		}
		...
		if (enable_usb) {
			ret = SAMPLE_COMM_AOV_BindUsb();
			if (ret != RK_SUCCESS) {
				printf("[%s()] bind usb failed!\n", __func__);
				break;
			}
		}
		if (enable_sdio) {
			ret = SAMPLE_COMM_AOV_BindSDIO();
			if (ret != RK_SUCCESS) {
				printf("[%s()] bind sdio failed!\n", __func__);
				break;
			}
		}
